...
public partial class user_sbupdate : System.Web.UI.Page
{
    ...
    public string Str_Moneysb = "", Str_Monesbmei = "", bizhong = "", zongji = "", zongjimei = "";//自行缴费
            ...
            if (mm != null)
            {
                if (mm.i_GuoJiId != null)
                {
                    ...
                    if (mm.i_UserTypeId != null)
                    {
                        ...
                        {
                            zhutiwenjian = "<a href='" + mm.nvc_ZhuTiFile + "'  title='点击查看'  target='_blank'>主体资格证书已上传</a>";
                            zhutifileIsOk = 1;
                        }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            string urlname = "";
            int flag = UpfileLoad(FileUpload4, 4, "File_Zscq/File_ShangBiao/", "商标注册证书复印件", ref urlname);
            if (flag == 1)
            {
                MessageBox.Show(Page, "文件大小不能超过 500KB");
                return;
            }
            if (flag == 2)
            {
                MessageBox.Show(Page, "文件格式仅限(.jpg  .png  .gif .pdf)!");
                return;
            }
            if (flag == 3)
            {
        ...
        if (hi_usertype.Value == "3")
        {
            ...
            else
            {
                ...
                {
                    string urlname = "";
                    int flag = UpfileLoad(FileUpload3, 3, "File_Zscq/File_ShangBiao/", "商标主体资格证明文件", ref urlname);
                    if (flag == 1)
                    {
                        MessageBox.Show(Page, "文件大小不能超过 500KB");
                        return;
        ...
        {
            MessageBox.Show(this.Page, "商标添加失败");
            //div_a.InnerHtml = "<script>alert('商标添加失败!');<script>";
        }
    }
    public int UpfileLoad(FileUpload fileup, int uploadtype, string pathname, string filename, ref string urlname)
    {
        try
        {
            ...
            {
                MessageBox.Show(this.Page, "文件格式仅限(.jpg  .png  .gif .pdf)!");
                return 2;
            }
        }
        catch
