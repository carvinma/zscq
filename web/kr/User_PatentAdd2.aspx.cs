...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    protected void ImgBtn_Add2_Click(object sender, ImageClickEventArgs e)
    {
                ...
                if (fileExt == ".xlsx" || fileExt == ".xls")
                {
                    ...
                    if (cells.MaxDataRow >= 8)
                    {
                        ...
                        for (int i = 8; i < cells.MaxDataRow + 1; i++)
                        {
                            ...
                            {
                                if (DALP.Patent_Select_Number(cells[i, 2].StringValue, DALN.Nationality_Select_Name(cells[i, 4].StringValue, "kr").i_Id, uId, 0) <= 0)
                                {
                                    #region
                                    t_Patent model = new t_Patent();
                                    model.i_MemberId = uId;//会员编号*
                                    model.nvc_Number = cells[i, 2].StringValue;//专利号*
                                    model.nvc_Name = cells[i, 3].StringValue;//특허명칭
                                    model.i_AuthorizeNationalityId = DALN.Nationality_Select_Name(cells[i, 4].StringValue, "kr").i_Id;//특허등록한 국가*
                                    model.i_PatentType = DALPT.PatentType_Select_GuoJiaIdAndName(model.i_AuthorizeNationalityId, cells[i, 5].StringValue, "kr").i_Id;//专利类型*
                                    ...
                                    {
                                        model.i_YaoQiuXiang = Convert.ToInt32(cells[i, 8].StringValue);//청구항
                                    }
                                    else
                                    {
                                        model.i_YaoQiuXiang = 0;
                                    }
                                    model.nvc_PatentHolder = cells[i, 9].StringValue;//특허권자
                                    if (cells[i, 10].StringValue != "")
                                    {
                                        model.i_PatentHolderNationalityId = DALN.Nationality_Select_Name(cells[i, 10].StringValue, "kr").i_Id;//특허권자 국적
                                    }
                                    ...
                                    {
                                        model.dt_AnnualFeePrevYear = Convert.ToDateTime(cells[i, 13].StringValue);//마지막으로 연회비 납부 마감일*
                                    }
                                    model.nvc_PriorityNumber = cells[i, 14].StringValue;//우선권 신청버호
                                    if (cells[i, 15].StringValue != "")
                                    {
                                        model.i_PriorityNationalityId = DALN.Nationality_Select_Name(cells[i, 15].StringValue, "kr").i_Id;//优先权申请国
                                    }
                                    if (cells[i, 16].StringValue != "")
                                    {
                                        model.dt_PriorityDate = Convert.ToDateTime(cells[i, 16].StringValue);//우선권 신청일
                                    }
                                    ...
                                    model.i_AnnualFeeYear = DALPT.PatentType_Select_GuoJiaIdAndName(model.i_AuthorizeNationalityId, cells[i, 5].StringValue, "kr").i_YouXiaoQi;//有效维持年限
                        ...
                        div_a.InnerHtml = "<script>alert('" + ok + "개 성공적으로 업레드 되였습니다 " + nos + "개 중복된 특허번호가 있습니다：" + nostr + "');location.href='User_PatentAddOk.aspx?AddType=성공 가져오기 특허&AddOkCount=" + ok + "';</script>";
                    }
                    else
                    {
                        div_a.InnerHtml = "<script>alert('데어터 없음')</script>";
                    }

                }
                else
                {
                    div_a.InnerHtml = "<script>alert('스프레드시트 업로드！')</script>";
                }
            }
        }
        catch (Exception ex)
        {
            ...
            div_a.InnerHtml = "<script>alert('데이터 이상')</script>";
        }
    }
}