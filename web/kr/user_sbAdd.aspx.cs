...
public partial class user_sbupdate : System.Web.UI.Page
{
    ...
    public string Str_Moneysb = "", Str_Monesbmei = "", bizhong = "",zongji="",zongjimei="";//스스로 비용을 납부하다
            ...
            if (mm != null)
            {
                if (mm.i_GuoJiId != null)
                {
                    ...
                    if (mm.i_UserTypeId != null)
                    {
                        ...
                        {
                            zhutiwenjian = "<a href='../" + mm.nvc_ZhuTiFile + "'  title='클릭하여 조사'  target='_blank'>신분을 증명할수있는 증명파일가 이미 업로드 되였습니다</a>";
                            zhutifileIsOk = 1;
                        }
    ...
    {       
           ...
           // sbguoji = Drp_GuoJi.SelectedValue;
               ...
               // model.i_GuoJiId = int.Parse(Drp_GuoJi.SelectedValue);
                ...
                {
                    string urlname = "";
                    int flag= UpfileLoad(FileUpload4, 4, "File_Zscq/File_ShangBiao/", "상표증서 복사건",ref urlname);
                    if(flag==1)
                    {
                        MessageBox.Show(Page, "파일크기는 500KB를 초과할수없습니다"); 
                        return;
                    }
                    if(flag==2)
                    {
                        MessageBox.Show(Page, "파일격식은 다만 (.jpg  .png  .gif .pdf)!여야 합니다!"); 
                        return;
                    }
                    if(flag==3)
                    {
                        MessageBox.Show(Page, "업로드 실패");
                        return;
                    }
                ...
                //else
                //{
                //    MessageBox.Show(Page, "请选择你要上传的文件");
                ...
                if (hi_usertype.Value == "3")
                {
                    ...
                    {
                        string urlname = "";
                        int flag = UpfileLoad(FileUpload3, 3, "File_Zscq/File_ShangBiao/", "주민등록증, 여권등 신분을 증명할수있는 증명서류", ref urlname);
                        if (flag == 1)
                        {
                            MessageBox.Show(Page, "파일크기는 500KB를 초과할수없습니다");
                            return;
                ...
                {
                    MessageBox.Show(this.Page, "상표추가 실패");
                    //div_a.InnerHtml = "<script>alert('상표추가 실패!');<script>";
                }                   
    }
    public int UpfileLoad(FileUpload fileup, int uploadtype, string pathname, string filename,ref string  urlname)
    {
        try
        {
            ...
            {
                MessageBox.Show(this.Page, "파일격식은 다만 (.jpg  .png  .gif .pdf)!여야 합니다!");
                return 2;
            }
        }
        catch
