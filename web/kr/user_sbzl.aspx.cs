...
public partial class user_sbzl : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>자료 수정_계정센터_상표시시템_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    public void Bind_Page_Info()//绑定基本信息
    {
        ...
        if (model != null)
        {
            ...
            {
                zhutiwenjian = "<a href='../" + model.nvc_ZhuTiFile + "' target='_blank' title='클릭하여 조사'>업로드 되였습니다(클릭하여 조사)</a><br><span  style=\"color:Red;\">증명문건이 변경하시면 다시 업로드해주세요</span>";
            }
        }
    }
    ...
    protected void Bt_Submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (result.Count() > 0)
            {
                div_a.InnerHtml = "<script>alert('메일박스 이미 사용되고！');</script>";
                return;
            }
            ...
            if (result2.Count() > 0)
            {
                div_a.InnerHtml = "<script>alert('핸드폰 번호 이미 사용되고！');</script>";
                return;
            }
        }
            if (hi_guoji.Value == "1")
            ...
            if (FileUpload3.HasFile)
            {
                ...
                if (fileNameExt == ".jpg" || fileNameExt == ".gif" || fileNameExt == ".png" || fileNameExt == ".pdf")
                {
                    ...
                    {
                        MessageBox.Show(this.Page, "파일크기는 500KB를 초과할수없습니다");
                        // div_a.InnerHtml = "<script>alert('文件大小不能超过 512KB');</script>";
                        return;
                    }
                    string toFileName = DateTime.Now.ToString("HHmmssff") + new Random().Next(100);
                    string path = System.AppDomain.CurrentDomain.BaseDirectory + "File_Zscq/File_ShangBiao/" + toFileName + "주체 자격 증명" + fileNameExt;
                    FileUpload3.SaveAs(path);
                    if (model.nvc_ZhuTiFile != null)
                    {
                        DelPhoto.DeleteFile(model.nvc_ZhuTiFile);
                    }
                    model.nvc_ZhuTiFile = "File_Zscq/File_ShangBiao/" + toFileName + "주체 자격 증명" + fileNameExt;
                }
                else
                {
                    MessageBox.ShowAndRedirect(this.Page, "파일 형식 오류！", "user_sbzl.aspx");
                    return;
                }
            }
            ...
            // div_a.InnerHtml = "<script>alert('수개성공');location.href='user_sbzl.aspx';</script>";
            ScriptManager.RegisterStartupScript(UpdatePanel2, this.GetType(), "", "<script>alert('수개성공');location.href='user_sbzl.aspx';</script>", false);
        }
        catch
        {
            ScriptManager.RegisterStartupScript(UpdatePanel2, this.GetType(), "", "<script>alert('매개 변수 잘못된 검토해 주십시오 후 제출！');location.href='user_sbzl.aspx';</script>", false);
