...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Request.Cookies["hqhtshop"] != null && Request.Cookies["hqhtshop"]["hqht_zl_uid"] != null && Request.Cookies["hqhtshop"]["hqht_zl_uid"].ToString() != "")
        {
            ...
            if (tn != null)
            {
                hi_paybizhong.Value = tn.nvc_JFBizhong;//费用缴纳国家외화종류
            }
            #endregion
            Delete_Patent();
            Bind_PatentList();
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            div_a.InnerHtml = "<script>alert('금액 총액은 0일수 없습니다')</script>";
            return;
        }
        #region 生成订单
        ...
        if (rb_kp2.Checked)
        {
            ...
            {
                string addr = "우편주소：" + address.countryName + "&nbsp;" + address.provinceName + "&nbsp;"
            + address.cityName + "&nbsp;" + address.areaName + "&nbsp;" + address.nvc_StreetAddress + "&nbsp;(" + address.nvc_Consignee + "&nbsp;)收"
            + "<br />联系电话：" + address.nvc_MobilePhone + "&nbsp;&nbsp;" + address.nvc_TelPhone + "<br />邮编编码：" + address.nvc_ZipCode + "<br />이메일：" + address.nvc_Email;
            ...
            if (flag != null && flag != "")
            {
                ...
                for (int j = 0; j < flaglist.Length; j++)
                {
                    if (flaglist[j] != "" || flaglist[j] != null)
                    {
                        ...
                        if (t != null && tc != null)
                        {
                            youhui += "+" + t.nvc_KRName + hi_paybizhong.Value + DALPO.GetPayMoneyByHuiLv(Convert.ToDecimal(t.i_Money1), "USD", hi_paybizhong.Value).ToString("0.00");
        ...
        if (Request.Cookies["hqht_patentidstr"] != null && Request.Cookies["hqht_patentidstr"].Value != "")
        {
            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null)
                    {
                        ...
                        InfoModer.nvc_Year = model.nvc_AnnualFeeNotPaymentYear;//납부년도

                        InfoModer.dm_ZhengFuGuiDingFei = model.dm_AnnualFeeNextPaymentMoney;
                        if (shuzu.IndexOf("," + model.i_AuthorizeNationalityId + ",") < 0)//授权国一样的话 只计算一次송금수수료
                        {
                        ...
                        InfoModer.nvc_PatentShouQuanGuo = DALN.Nationality_Select_Id(model.i_AuthorizeNationalityId).nvc_HanName;
            ...
            strzl.Append("<tr>");
            strzl.Append("<td height='35' align='center' bgcolor='#FFFFFF'>특허번호</td>");
            strzl.Append("<td width='12%'align='center' bgcolor='#FFFFFF'>특허명칭</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>특허권자</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>인증 나라</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>납부년도</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>납부기한</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>납부금액</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>외화종류</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>안건번호</td>");
            strzl.Append("</tr>");
            ...
            zongji.Append("<td width='200' align='right'> 관비：</td><td width='110'>" + OrderModer.dm_SetMoney.ToString("0.00") + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 추가 지체료：</td><td width='110'>" + OrderModer.dm_ZhiNaJin + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 대리비용：</td><td width='110'>" + OrderModer.dm_AgentServiceFee.ToString("0.00") + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 가산료：</td><td width='110'>" + OrderModer.dm_ChaoXiangFee.ToString("0.00") + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'></td><td width='110'>합계：</td><td width='30' style='color: red;'>" + OrderModer.nvc_CurrencyType + "</td><td style='width: 100px;color: red;'>" + (Convert.ToDecimal(OrderModer.dm_SetMoney + OrderModer.dm_AgentServiceFee + OrderModer.dm_ChaoXiangFee + OrderModer.dm_ZhiNaJin)).ToString("0.00") + "</td>"); 
            zongji.Append("</tr>");
            ...
            { 
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 할인권 사용：</td><td width='110'>" + OrderModer.dm_YouHui + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
			}
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 송금수수료：</td><td width='110'>" + OrderModer.dm_RemittanceFee + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            ...
            zongji.Append("<table width='440' cellspacing='0' cellpadding='0' border='0'><tbody>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'></td><td width='110'>합계：</td><td width='30' style='color: red;'>" + OrderModer.nvc_CurrencyType + "</td><td style='width: 100px;color: red;'>" + OrderModer.dm_TotalMoney.ToString("0.00") + "</td>");
            zongji.Append("</tr>");
            zongji.Append("</tbody></table>");
            ...
            str = str.Replace("$order_type", "특허 주문");
            str = str.Replace("$order_stat", DALPO.GetStatus(OrderModer.i_Status,"kr"));
            str = str.Replace("$user_num", tm.nvc_UserNum);
            str = str.Replace("$true_name", tm.nvc_RealName);
            str = str.Replace("$user_counrtyInfo", DALN.Nationality_Select_Id(tm.i_GuoJiId).nvc_HanName);
            ...
            str = str.Replace("$fp", OrderModer.i_IsFaPiao == 1 ? "예" : "아니요");
            ...
            if (OrderModer.nvc_BankId != null && OrderModer.nvc_BankId.ToString()!="")
            {
                ...
                yinhang.Append("<tr>");
				yinhang.Append("<td width='24%' height='28' align='center' bgcolor='#FFFFFF'><strong>은행 세부 사항</strong></td>");
                yinhang.Append("<td width='20%' height='28' align='center' bgcolor='#FFFFFF'><strong>SWIFT CODE</strong></td>");
                yinhang.Append("<td width='28%' height='28' align='center' bgcolor='#FFFFFF'><strong>계정 이름</strong></td>");
                yinhang.Append("<td width='28%' height='28' align='center' bgcolor='#FFFFFF'><strong>계좌 번호</strong></td>");
                yinhang.Append("</tr>");
            ...
            if (remail == 1)//如果会员接收邮件并且后台设置发送邮件
            {
                ...
                string send_fromtitle = "Global Patent-특허시스템-주문 제출 성공";//标题
            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null)
                    {
                        ...
                        sb_patent.Append("<td align='center' bgcolor='#FFFFFF'>" + DALN.Nationality_Select_Id(model.i_AuthorizeNationalityId).nvc_HanName + "</td>");
                        ...
                        sb_patent.Append("<td align='center' bgcolor='#FFFFFF'><a href='User_PatentInfo.aspx?pId=" + pid + "&href=" + href + "' class='ac5t'>보기</a> <a href=" + "'?pagetype=del&cs=" + pid + "," + number + "' class='ac5t'>삭제</a></td>");
                        sb_patent.Append("</tr>");
                        ...
                        sb_patent_1.Append("<td align='center' bgcolor='#FFFFFF'>" + DALN.Nationality_Select_Id(model.i_AuthorizeNationalityId).nvc_HanName + "</td>");
                        ...
                        if (model.i_AuthorizeNationalityId != 1)
                        {
                            ...
                            {
                                //税率=(（관비+대리비용+송금수수료+滞纳金+가산료）换算成人民币-200 )*33/1000
                        ...
                        //有个问题：同一授权国下的几个专利송금수수료不一样的时候按哪个来
                    }
                    #endregion
                }
            }
