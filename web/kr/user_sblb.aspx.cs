...
public partial class user_sblb : System.Web.UI.Page
{
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            div_a.InnerHtml = "<script>alert('납부 상표 선택하십시오.');</script>";

        }
    }
    public string ZTFileImg(object zhuti,object sbid)
    {
        string aa = "<a href='user_sbzl.aspx'>미 업로드</a>";
        var m = DALM.Member_Select_Id(UserId);
        if (m != null)
        {
            if (m.i_UserTypeId == 3)
            {
                if (zhuti != null && zhuti != "")
                {
                    return "<a href='../" + zhuti + "' target='_blank'><img src='../" + zhuti + "'  width='20' title='주민등록증, 여권등 신분을 증명할수있는 증명서류' border='0'/></a>";
                }
                else
                {
                    return "<a href='user_sbupdate.aspx?t_r_id=" + sbid + "' target='_blank'>미 업로드</a>";
                }
            }
            else
            {
                ...
                {
                    return "<a href='../" + m.nvc_ZhuTiFile + "'  target='_blank'><img src='../" + m.nvc_ZhuTiFile + "'  width='20' title='주민등록증, 여권등 신분을 증명할수있는 증명서류' border='0'/></a>";
                }
                else
                {
                    return "<a href='user_sbzl.aspx'>미 업로드</a>";
                }
            }
        }
        return aa;
    ...
    protected void ImageButton5_Click(object sender, ImageClickEventArgs e)
    {
        if (Request.Cookies["hqht_Trademarktidstr"] != null && Request.Cookies["hqht_Trademarktidstr"].Value != "")
        {
            ...
            Bind_Rpt_Trademark();
            div_html.InnerHtml = "<script>alert('삭재성공')</script>";
        }
        else
        {
            div_html.InnerHtml = "<script>alert('좀 삭제할 상표 선택')</script>";
        }
    }
