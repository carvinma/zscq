...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    void Add_OnlineMsg()
    {
        ...
        {
            div_a.InnerHtml = "<script>alert(\"정확한 이메일 입력하십시오.！\");</script>";
            return;
        }
        ...
        if (DALOM.OnlineMessage_Add(model) > 0)
            div_a.InnerHtml = "<script>alert(\"문제는 이미 고개센터로  발송하였습니다 24시간이내에 답변드리겠습니다  즉지검색하기를  바랍니다！\");window.location='user_patentmessage.aspx'</script>";
        else
            div_a.InnerHtml = "<script>alert(\"뎃글 발송실페！\");</script>";
    }
