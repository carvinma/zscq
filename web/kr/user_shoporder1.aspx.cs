...
public partial class user_shoporder1 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            DALN.Nationality_Bind_DDL(ddl_guojia,"kr");
            Bind_Page_Member();
            Bind_Page_Value();
                ...
                foreach (var q in ShopCart)
                {
                    if (q != "")
                    {
                        ...
                        if (model != null)
                        {
                            ...
                            td_ShopCart.Append("<td width='69' align='center' bgcolor='#FFFFFF'><a target=\"_blank\" href='user_shoporder1.aspx?action=del&cid=" + model.i_Id + "&pid=" + model.i_Type + "' class='ac5t'>삭제</a></td>");
                            td_ShopCart.Append("</tr>");
                        }
                    }
                }
    ...
    void Delete_Address()//删除地址
    {
        if (Request.QueryString["PageType"].ToString() == "del")
        {
            ...
            {
                div_a.InnerHtml = "<script>alert('의외의 사고가 발생한，이 주소로 삭제할 수 없습니다！')</script>";
            }
            else
            {
                #region 如果删除了默认的地址
    ...
    void Mr_Address()//默认
    {
        if (Request.QueryString["PageType"].ToString() == "mr")
        {
            ...
            {
                div_a.InnerHtml = "<script>alert('의외의 사고가 발생한！')</script>";
            }
            else
            {
                Response.Redirect("user_shoporder1.aspx");
    ...
    bool AddCounp(vw_Member v_member, int Pid, int number)
    {
        try
        {
            ...
            BLLC.Coupon_ForAdd(number, DateTime.Now.AddMonths(Convert.ToInt32(model.i_TimeLength)).ToString(), model.i_CouponTypeId.ToString(), v_member.i_Id, "商城购买");
            return true;
        }
        catch { return false; }
    }
        ...
        try
        {
            ...
            if (m_member != null & m_mobile != null)
            {
                if (m_mobile.i_Integral >= CCart.Set_ShopCartIntegral())
                {
                        ...
                        if (q != "")
                        {
                            ...
                            if (model_ip != null)
                            {
                                if (model_ip.i_Type == 1)
                                {
                                    ...
                                    {
                                        div_a.InnerHtml = "<script>alert('일단 주소 선택！');window.location='user_shoporder1.aspx';</script>";
                                        return;
                                    }
                                    #endregion
                                }
                                ...
                                if (model_ip.i_Type == 3)
                                {
                                    ...
                                    {
                                        div_a.InnerHtml = "<script>alert('회원 등급 상품을 구매할 수 한 가지！');window.location='user_shoporder1.aspx';</script>";
                                        return;
                                    }
                                    if (m_member.i_GradeId >= model_ip.i_Grade)
                                    {
                                        div_a.InnerHtml = "<script>alert('이 회원 등급 상품 구매 할 필요가 없다！');window.location='user_shoporder1.aspx';</script>";
                                        return;
                                    }
                                    if (model_ip.i_Grade - m_member.i_GradeId > 1)
                                    {
                                        div_a.InnerHtml = "<script>alert('못 사다 이 회원 등급 상품！');window.location='user_shoporder1.aspx';</script>";
                                    }
                                    #endregion
                                }
                    ...
                    BLLIOON.IntegralOrderOperateNote_Add("商城下单", "", MemberId, model.i_Id);
                    CCart.Clear_ShopCart();
                ...
                {
                    div_a.InnerHtml = "<script>alert('포인트 부족하다.');window.location='user_shoporder1.aspx';</script>";
                    return;
                }
            }
            else
            {
                div_a.InnerHtml = "<script>alert('로그인 이미 기한이 지나서！');window.location='index.aspx?pageurl2='" + HttpUtility.UrlEncode(Request.Url.ToString()) + "</script>";
            }
        }
        catch
        {
