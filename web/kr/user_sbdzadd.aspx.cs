...
public partial class user_sbdzadd : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>우편주소_계정센터_특허시스템_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
        ...
        if (Hi_AddressId.Value == "0")
        {
            switch (DALRA.ReceiveAddress_Add(model))
            {
                ...
                case 1:
                    ...
                    div_a.InnerHtml = "<script>alert('추가 성공！'); window.location='user_sbdz.aspx'</script>";
                    break;
                case 2:
        ...
        else
        {
            switch (DALRA.ReceiveAddress_Update(model))
            {
                ...
                case 1:
                    ...
                    UserLog.AddUserLog(Hi_MemberId.Value, "商标系统", "添加地址");
                    div_a.InnerHtml = "<script>alert('수개성공'); window.location='user_sbdz.aspx'</script>";
                    break;
                case 2:
                    div_a.InnerHtml = "<script>alert('동일한 수신인과 수신주소  중복추가할수 없습니다!');</script>";
                    break;
