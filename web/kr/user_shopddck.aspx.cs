...
public partial class user_shopddck : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>주문 정보_나의 바꿈 주문_상가 시스템_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    public void Bind_Page_Value()//绑定所有地址
    {
        if (Request.QueryString["orderid"] != null && Request.QueryString["orderid"].ToString() != "")
        {
            ...
            if (model != null && model.i_MemberId.ToString() == Hi_MemberId.Value)
            {
                ...
                Address += string.IsNullOrWhiteSpace(model.nvc_HanName) == true ? "" : model.nvc_HanName + "&nbsp;&nbsp;";
                ...
                OrderStatus = DALIO.Set_IntegralOrderState(model.i_State,"kr");
