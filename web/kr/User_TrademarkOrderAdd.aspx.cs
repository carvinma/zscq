    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {          
                ...
                if (flag != null && flag != "")
                {
                    ...
                    for (int j = 0; j < flaglist.Length; j++)
                    {
                        if (flaglist[j] != "" || flaglist[j] != null)
                        {
                            ...
                            {  
                                yhqid += tc.i_Id + ",";
                                youhui += "+" + t.nvc_KRName + bizhong + t.i_Money;
           ...
           // OrderModer.nvc_YouHUiQuan = "0";
            ...
            if (sbnum == 0)
            {
                div_a.InnerHtml = "<script>alert('상표를 지불하고 싶지 않았다！');localtion.href='user_sblb.aspx';</script>";
            }
            else
            {
                if (emalladdress != "")
                {
                    ...
                    if (input_payway.Value == "线下汇款")
                    {
                       // huikuanbankinfo = "";
                        ...
                        string str1 = "다음과 같이 회사의 은행 계좌 정보는<br/>";
                        for (int i = 0; i < OrderModer.nvc_BankId.Split(',').Length; i++)
                        {
                            ...
                            if (tb != null)
                            {
								...
								str1 += "<br/><font color='red'>은행 세부 사항：" + tb.nvc_BankDetails + "<br/>SWIFT CODE：" + tb.nvc_SwiftCode + "<br/>계정 이름：" + tb.nvc_AccountName + "&nbsp;<br/>계좌 번호：" + tb.nvc_BankNumber + "</font><br/>";
                            }
                        }
                        huikuanbankinfo = str1;
                    }

BLLE.Email_Add(emalladdress, "상표 지불 명령", "안녕하세요! 당신은 주문 번호를 지불해야：" + OrderModer.nvc_OrderNumber + " <br/>  시간：" + OrderModer.dt_AddTime + "  <br/>  당신은 라인을 지불하는 것을 선택，" + huikuanbankinfo + " <br/><br/> 상표를 지불 한 비용의 세부 사항：<br/>" + hi_feeinfo.Value + "<br/>24 근무 시간 이내에 지불하십시오! 당신은 질문이있는 경우에는,Global Patent로 연락 주시기 바랍니다！<br/>전화：86-10-84505596<br/>E-MAIL：pat-annuity@hqht-online.com", uId, ref states, "kr");
                }
        ...
        if (Request.Cookies["hqht_Trademarktidstr"] != null && Request.Cookies["hqht_Trademarktidstr"].Value != "")
        {
            ...
            if (muser != null)
            {
                if (muser.i_UserTypeId == 3)
                {
                    ...
                    {
                        MessageBox.ShowAndRedirect(this.Page, "사용자 자료 작성 안 전체, 일단 보완 자료！", "user_sbzl.aspx");
                        return;
                    }
                }
                ...
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null && muser != null)
                    {
                        if (muser.i_UserTypeId == 3)
                        {
                            ...
                            {
                                MessageBox.ShowAndRedirect(this.Page, model.nvc_SBRegNum + "이 상표는 업로드 주요 자격증 없이 좀 상표 목록 안에 탭 업로드","user_sblb.aspx");
                                return;
                            }
                        }
                        else if (muser.i_UserTypeId != 3)
                        {
                            ...
                            {
                                MessageBox.ShowAndRedirect(this.Page, model.nvc_SBRegNum + "의 상표증서 복사건이 업로드하지않았습니다  업로드하세요", "user_sbzl.aspx");
                                return;
                            }
                        }
                        if (model.nvc_SBFile == null || model.nvc_SBFile == "")
                        {
                            MessageBox.ShowAndRedirect(this.Page, model.nvc_SBRegNum + "의 상표증서 복사건이 업로드하지않았습니다  업로드하세요", "user_sblb.aspx");
                            return;
                        }
                    }
                }
            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null)
                    {
                        ...
                        //sb_trademrk.Append("<td width='76' align='center' bgcolor='#FFFFFF'>" + (model.i_IsPayState.ToString() == "2" ? "이미 비용, 기다리다, 다음에 납부" : (model.i_IsPayState.ToString() == "3" ? "무효" : "미 납부")) + "</td>");
                        sb_trademrk.Append("<td align='center' bgcolor='#FFFFFF'>" + model.nvc_SbDaoqiTime + "</td>");
                        sb_trademrk.Append("<td align='center' bgcolor='#FFFFFF' class=\"hidesbinfo\"><a href='user_sbck.aspx?t_r_id=" + tid + "&href=" + href + "' class='ac5t'>보기</a> <a href=" + "'?pagetype=del&cs=" + tid + "," + number + "' class='ac5t'>삭제</a></td>");
                        sb_trademrk.Append("</tr>");
                    }
                }
            }
                    ...
                    if (na != null)
                    {
                        ...
                        if (na.nvc_Name == "中国")
                        {
                           ...
                           TMDaiLi = dalifee * sbdailinum;
                            ...
                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">상표갱신등록관납료：</td><td width=\"110\">" + (model1.dm_TrademarkMoney).ToString("0.00") + "*" + sbnum + "</td><td width=\"30\"></td><td width=\"100\"></td></tr>");
                            if (zhinajinnum != 0)
                            {
                                Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">추가 지체료：</td><td width=\"110\">" + (model1.dm_TMZhiNaJin).ToString("0.00") + "*" + zhinajinnum + "</td><td width=\"30\"></td><td width=\"100\"></td></tr>");
                            }
                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">대리 비용：</td><td width=\"110\">" + (dalifee).ToString("0.00") + "*" + sbdailinum + "</td><td width=\"30\"></td><td width=\"100\"></td></tr>");

                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\"></td><td width=\"110\" align=\"right\">합계：</td><td width=\"30\">CNY</td><td width=\"100\" align=\"left\">" + totalmoney + "</td></tr>");

                            Str_AllMoney.Append("<tr align=\"left\"><td width=\"200\" align=\"right\"></td><td width=\"110\" align=\"right\">합계：</td><td style=\"color:red;\" width=\"30\">CNY</td><td style='color:red;width:100px;' id='allmoney'  align=\"left\">" + totalmoney + "</td></tr>");
                        }
                        else
                         {
                             //Bind_Rpt_BankList(1,0);//国外
                            ...
                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">상표갱신등록관납료：</td><td width=\"110\">" + (model1.dm_TrademarkMoney).ToString("0.00") + "*" + sbnum + "</td><td width=\"30\"></td><td style=\"width:100px;\"></td></tr>");
                            if (zhinajinnum != 0)
                            {
                                Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">추가 지체료：</td><td width=\"110\">" + (model1.dm_TMZhiNaJin).ToString("0.00") + "*" + zhinajinnum + "</td><td width=\"30\"></td><td style=\"width:100px;\"></td></tr>");
                            }
                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">대리 비용：</td><td width=\"110\">" + (dalifee).ToString("0.00") + "*" + sbdailinum + "</td><td width=\"30\"></td><td style=\"width:100px;\"></td></tr>");

                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\"></td><td width=\"110\" align=\"right\">합계：</td><td width=\"30\">CNY</td><td style=\"width:100px;\" align=\"left\">" + totalmoney + "</td></tr>");

                            Str_AllMoney.Append("<tr align=\"left\"><td width=\"200\" align=\"right\"></td><td width=\"110\" align=\"right\">합계：</td><td style='color:red;' width=\"30\">CNY</td><td style='color:red;' width=\"100\" id='allmoney'  align=\"left\">" + totalmoney + "</td></tr>");
    ...
    void Bind_Drp_PGuoJiaType()// 绑定国籍
    {
        ...
        // ListItem item = new ListItem("선택", "0");
