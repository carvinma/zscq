                    ...
                    { 
                        for (int i = 6; i < cells.MaxDataRow + 1; i++)
                        {
                            ...
                            if (td == null)
                            {
                                ...
                                {
                                    info = "제 " + i + "구절의 시간격식이 틀렸음으로 업로드가 중단되였습니다. " + oknum + "개의 데이터가 성공적으로 업로드하였습니다";
                                }
                                
                                model.i_MiaoShuType = 1;
                                if (miaoshu.IndexOf("문자") >= 0)
                                {
                                    model.i_MiaoShuType = 1;
                                }
                                if (miaoshu.IndexOf("도형") >= 0)
                                {
                                    model.i_MiaoShuType = 2;
                                }
                                if (miaoshu.IndexOf("와") > 0)
                                {
                                    model.i_MiaoShuType = 3;
                                }
                                model.i_JiaoFeiType = 2;//默认是위탁 납부
                        ...
                        div_a.InnerHtml = "<script>alert('" + oknum + "개 성공적으로 업레드 되였습니다" + info + " " + sbnum + "개 중복된 특허번호가 있습니다：" + fstr + "');location.href='user_sbAddOk.aspx?shu=" + oknum + "&ok=success';</script>";//  Response.Redirect("user_sbAddOk.aspx?shu=" + oknum + "&ok=success");
