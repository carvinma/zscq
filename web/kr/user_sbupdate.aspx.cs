...
public partial class user_sbupdate : System.Web.UI.Page
{
    ...
    public string JiaoFeiType = "0", Sb_num = "", shenhe = "미심사", jiaofei = "미 납부";
    ...
    public string Str_Moneysb = "0", Str_Monesbmei = "", Str_Zhinajin = "", bizhong = "", zongji = "", zongjimei = "";//스스로 비용을 납부하다
        ...
        else
        {
            ...
            if (Request.QueryString["t_r_id"] != null && Request.QueryString["t_r_id"].ToString() != "")
            {
                ...
                if (model != null)
                {
                    ...
                    if (mm != null)
                    {
                        ...
                        if (mm.i_UserTypeId != null)
                        {
                            if (mm.i_UserTypeId == 1)
                            {
                                ...
                                {
                                    zhutiwenjian = "<a href='../" + mm.nvc_ZhuTiFile + "' title='클릭하여 조사'  target='_blank'>신분을 증명할수있는 증명파일가 이미 업로드 되였습니다</a><br><span  style=\"color:Red;\">증명문건이 변경하시면 다시 업로드해주세요</span>";
                                    zhitiOk = 1;
                                }
                                else
                                {
                            ...
                            if (mm.i_UserTypeId == 3)
                            {
                                ...
                                {
                                    zhutiwenjian = "<a href='../" + model.nvc_ZhuTiFile + "' title='클릭하여 조사'  target='_blank'>신분을 증명할수있는 증명파일가 이미 업로드 되였습니다</a><br><span  style=\"color:Red;\">증명문건이 변경하시면 다시 업로드해주세요</span>";
                                    zhitiOk = 1;
                                }
                                #region 제삼방 정보
                                t_anjuanhao.Value = model.nvc_CFaxnumber;//안건번호
                    ...
                    {
                        Sbfile = "<a href='../" + model.nvc_SBFile + "' title='클릭하여 조사'  target='_blank'>상표 등록 증명서 복사본을 업로드됨</a><br><span  style=\"color:Red;\">증명문건이 변경하시면 다시 업로드해주세요</span>";
                        sbfileok = 1;
                    }
                    else
                    {
                    ...
                    {
                        //shangchuan = 2;//是否上传
                        shenqingfile = "<a href='../" + model.nvc_ShenQingShu + "' title='클릭하여 조사' target='_blank'>신청서 업로드됨</a>&nbsp;&nbsp;<span  style=\"color:Red;\">증명문건이 변경하시면 다시 업로드해주세요</span>";
                    ...
                    if (model.i_JiaoFeiType == 2 || mm.i_UserTypeId == 3)
                    {
                        ...
                        {
                            weituofile = "<a href='../" + model.nvc_WeituoFile + "' title='클릭하여 조사' target='_blank'>의뢰서 업로드됨</a>&nbsp;&nbsp;<span  style=\"color:Red;\">증명문건이 변경하시면 다시 업로드해주세요</span>";
                            weituoshu = 1;
                            //Button1.Visible = false;
                        }
                        else
                    ...
                    shengtime = model.i_ShengDays.ToString() + "날";
                    if (model.i_ShengDays < 0)
                    {
                        shengtime = "만료";
            ...
            if (Request.QueryString["t_r_id"] != null && Request.QueryString["t_r_id"].ToString() != "")
            {
                ...
                //{
                    if (FileUpload4.HasFile)
                    {
                        ...
                        if (i == 1)
                        {
                            div_a.InnerHtml = "<script>alert('파일크기는 500KB를 초과할수없습니다');location.href='" + url + "';</script>";
                            return;
                        }
                        if (i == 2)
                        {
                            div_a.InnerHtml = "<script>alert('파일격식은 다만 (.jpg  .png  .gif .pdf)!여야 합니다!');location.href='" + url + "';</script>";
                            return;
                        }
                    ...
                    //else
                    //{
                    //    div_a.InnerHtml = "<script>alert('상표 등록 증명서 복사본을 올려 주세요.');location.href='" + url + "';</script>";
                ...
                {
                    #region 제삼방 정보
                    model.nvc_CFaxnumber = t_anjuanhao.Value;//안건번호
                    ...
                    //    else
                    //    {
                    //        MessageBox.Show(this.Page, "주요 자격증 올려 주세요.");
                    //        //  div_a.InnerHtml = "<script>alert('너 업로드할 파일을 선택하십시오.');</script>";
                    //        return;
                    //    }
                    //}
                    //else
                ...
                {
                    div_a.InnerHtml = "<script>alert('정보 업데이트 실패!');<script>";
                }
            }
        }
    }
