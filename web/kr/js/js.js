...
function commentallpage(vpage) {
  obj('Y_Foot_Page1').innerHTML = "<img alt=\"기다려 주십시오\" src=\"images/loading.gif\" style=\"margin-left:350px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function zxpage(vpage) {
  obj('Y_Foot_Page2').innerHTML = "<img alt=\"기다려 주십시오\" src=\"images/loading.gif\" style=\"margin-left:340px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function buynotepage(vpage) {
  obj('divbuynote').innerHTML = "<img alt=\"기다려 주십시오\" src=\"images/loading.gif\" style=\"margin-left:340px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function commentsubmit() {
  if (obj("t_content").value == "문명 인터넷 리 지 발언하다。" || obj("t_content").value.toString() == "") {
    obj('t_content').focus();
...
//登录
function GetLogin() {
  if (obj("text_username").value == "아이디" || obj("text_username").value.toString() == "") {
    obj('text_username').focus();
    changecolor('div_username', 0, "명칭을  입력해주세요！");
    return false;
  }
  if (obj("TB_pwd").value == "") {
    ...
    obj('TB_pwd').focus();
    changecolor('div_pwd', 0, "비밀번호 입력해주세요！");
    return false;
  }
  if (obj("text_v")) {
    if (obj("text_v").value == "인정번호" || obj("text_v").value.toString() == "") {
      obj('text_v').focus();
      changecolor('div_checkcode', 0, "인증 번호 입력해주세요！");
      return false;
    }
  }
...
function checkonsubmit() {
  if (!obj("cb_Agreement").checked) {
    alert("죄송합니다. 만약 꼭 이 책을 위해 사이트 등록 회원이, 당신은 반드시 동의 회원 등록 프로토콜！");
    return false;
  }
  //    if (obj("text_regmail").value.toString() == "" || obj('text_regmail').value == "邮箱/手机") {
  //        obj("text_regmail").focus();
  //        changecolor('div_regmail', 0, "명칭을  입력해주세요！", 'li_tixing2');
  //        return false;
  //        }
  ...
  if (reg.test(obj("text_regmail").value.toString()) == false && reg3.test(obj('text_regmail').value) == false) {
    ...
    changecolor('div_regmail', 0, "잘못된 사용자 형식！", 'li_tixing2');
    return false;
  }
  ...
  if (obj('Hi_Repeat').value == "1") {
    obj("text_regmail").select();
    changecolor('div_regmail', 0, obj("text_regmail").value + "이미 존재합니다.", 'li_tixing2');
    return false;
  }
  ...
  if (obj("pwd_regpwd").value.toString() == "") {
    ...
    changecolor('div_regpwd', 0, "비밀번호 입력해주세요！");
    return false;
  }
  if (obj("pwd_regpwd").value.length < 6 || obj("pwd_regpwd").value.length > 21) {
    ...
    obj('text_regpwd').style.display = 'none';
    changecolor('div_regpwd', 0, "비밀번호 길이 여섯 ~ 21 분 사이에！");
    return false;
  }
  if (obj("pwd_regpwdok").value.toString() == "") {
    ...
    changecolor('div_regpwdok', 0, "부디 다시 암호를 입력하십시오！");
    return false;
  }
  if (obj("pwd_regpwd").value.toString() != obj("pwd_regpwdok").value.toString()) {
    ...
    obj('text_regpwdok').style.display = 'none';
    changecolor('div_regpwdok', 0, "두 번 입력한 암호를 다르다！");
    return false;
  }
  if (obj('text_regcheckcode').value == "" || obj('text_regcheckcode').value == "인정번호") {
    obj("text_regcheckcode").focus();
    changecolor('div_regcheckcode', 0, "인증 번호 입력해주세요！");
    return false;
  }
  obj('IBt_Register').innerHTML = "지금 등록…";
}
