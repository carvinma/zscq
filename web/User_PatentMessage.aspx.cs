...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    void Add_OnlineMsg()
    {
        ...
        if (DALOM.OnlineMessage_Add(model) > 0)
            div_a.InnerHtml = "<script>alert(\"问题已发送到客服中心，我们将在24小时之内回复您，请注意及时登录查询！\");window.location='user_patentmessage.aspx'</script>";
        else
            div_a.InnerHtml = "<script>alert(\"留言发送失败！\");</script>";
    }
