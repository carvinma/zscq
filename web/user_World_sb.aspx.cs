   ...
   public int trademarkId =0;
    ...
    public void Bind_Page_Type()// 绑定页面信息
    {
        if (Request.QueryString["id"] != null && Request.QueryString["id"].ToString() != "")
        {
                ...
                foreach (var item in iquery)
                {
                    ...
                    if (Request.QueryString["zixing"] != null)
                    {
                        if (Request.QueryString["zixing"] != "")
                        {
                            ...
                            {
                                MessageBox.ShowAndRedirect(this.Page, "数据异常,将要跳转到列表页...","user_sblb.aspx"); 
                            }
                        }
                    }
                    else
            ...
            #endregion
            string path = Server.MapPath("File_Zscq/" + sbnumber + "商标续展注册申请书.doc");
            doc.Save(path, SaveFormat.Doc); //保存为doc
            Response.Redirect("File_Zscq/" + sbnumber + "商标续展注册申请书.doc"); 
        }
    }
