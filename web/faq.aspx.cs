...
public partial class faq : System.Web.UI.Page
{
    ...
    {
        t_PageInfo model = new dal_PageInfo().PageInfo_Select_Id(8);
    ...
    void Bind_Rpt_List()
    {
        ...
        var iquery = from i in dvdc.vw_OnlineMessage where i.nvc_Language=="cn" select i;
    ...
    protected void ibtn_Add_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (DALOM.OnlineMessage_Add(model) > 0)
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.GetType(), "msg", "<script>alert(\"问题已发送到客服中心，我们将在24小时之内回复您，请注意及时登录查询！\");window.location='faq.aspx';</script>", false);
        else
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.GetType(), "msg", "<script>alert(\"留言发送失败！\");</script>", false);
