...
public partial class user_sbly : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>留言反馈_服务中心_商标系统_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    void Add_OnlineMsg()
    {
        ...
        {
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.GetType(), "msg", "<script>alert(\"您输入的字符不符合规范,请核对后重新输入！\");</script>", false);
            //  div_a.InnerHtml = "<script>alert('您输入的字符不符合规范,请核对后重新输入！');</script>";
            return;
        }
        ...
        if (DALOM.OnlineMessage_Add(model) > 0)
        {
            ...
            System.Web.UI.ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "success", "alert(\"问题已发送到客服中心，我们将在24小时之内回复您，请注意及时登录查询！\");window.location='user_sbly.aspx'", true);
        }
        else
        {
            System.Web.UI.ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "success", "alert(\"留言发送失败！\");", true);
        }
    }
