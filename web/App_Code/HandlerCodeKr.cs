...
public class HandlerCodeKr
{
    ...
    public void selshouquanguo(HttpContext context)//专利授权国
    {
        ...
        foreach (var v in iquery)
        {
            if (ck == v.i_Id)
            {
                sb.Append("<option selected=\"selected\" value=\"" + v.i_Id + "\">" + v.nvc_HanName + "</option>");
            }
            else
            {
                sb.Append("<option value=\"" + v.i_Id + "\">" + v.nvc_HanName + "</option>");
            }
        }
    ...
    public void selpatenttype(HttpContext context)//专利类型
    {
        ...
        if (guojiid != 0)
        {
            foreach (var v in iquery)
            {
                if (ck == v.i_Id)
                {
                    sb.Append("<option selected=\"selected\" value=\"" + v.i_Id + "\">" + v.nvc_koreanName + "</option>");
                }
                else
                {
                    sb.Append("<option value=\"" + v.i_Id + "\">" + v.nvc_koreanName + "</option>");
                }
            }
        }
        context.Response.Write(sb);
    ...
    public void login(HttpContext context)//登录
    {
        ...
        {
            context.Response.Write("패스워드 이미 효력을"); return;
        }
        string l_codes = context.Session["Validate"].ToString();
        ...
        if (l_code.Trim() == l_codes.Trim())
        {
            if (StringHelper.CheckJsAndHtmlKeyWord(l_uPwd) == false)
            {
                ...
                {
                    context.Response.Write("아이디 또는 비밀번호를 다시 확인하세요");
                }
            }
            else
            {
                context.Response.Write("잘못된 문자를 입력하지 마십시오");
            }
        }
        else
        {
            context.Response.Write("잘못된 인증 코드");
        }
    }
        ...
        if (StringHelper.CheckSqlKeyWord(r_uName) == false && StringHelper.CheckJsAndHtmlKeyWord(r_uName) == false)
        {
            ...
            switch (r_pageFlag)
            {
                ...
                case "sb":
                    ...
                    break;
                default:
                    context.Response.Write("누락 된 유형"); context.Response.End();
                    break;
            }
            ...
            switch (ret)
            {
                case 1:
                    ...
                    if (count > 0)
                    {
                        context.Response.Write("2;;;;;신청완료되였습니다 " + count + "개의 상품권 얻었습니다 기한일 전에 사용해주세요");
                    }
                    else
                    {
                        context.Response.Write("1;;;;;등록 성공");
                    }
                    break;
                case 2:
                    context.Response.Write("0;;;;;사용자 이름이 이미 존재합니다");
                    break;
                case 3:
                    context.Response.Write("0;;;;;E-메일이 등록되어 있습니다");
                    break;
                case 4:
                    context.Response.Write("0;;;;;전화 번호가 이미 존재합니다");
                    break;
                case 5:
                    context.Response.Write("0;;;;;사용자 ID가 이미 존재합니다");
                    break;
                default:
                    context.Response.Write("0;;;;;시스템이 사용 중입니다, 기다려주세요...");
                    break;
            }
        }
        else
        {
            context.Response.Write("0;;;;;잘못된 문자를 입력하지 마십시오");
        }
    }
        ...
        if (context.Request["SelectType"].ToString() == "maxjiaofei")
        {
            ...
            if (iquery.Count() > 0)
            {
                ...
                foreach (var item in iquery)
                {
                    ...
                    {
                        info = item.nvc_SBRegNum + "이 만기 상표 긴 시간" + item.i_ShengDays + "천일, 납부 기한까지";
                    }
                }
    ...
    public void findpwd(HttpContext context)//找回密码
    {
        ...
        {
            context.Response.Write("0;;;;;파라미터 이상 요청！");
            return;
        }
        if (context.Session["Validate"] != null)
        {
            ...
            dal_Member DALM = new dal_Member();
            string scontent = "존경하는고객님 현재비밀번호 찾음을 수행하고있습니다  인증코드번호:" + checkv + ". 인증코드번호의 기간경과를 피면하기 위하여 즉시인증코드를 입력하여 비밀번호를 수정하여주세요";
            string scontent2 = "존경하는고객님 현재비밀번호 찾음을 수행하고있습니다  인증코드번호:" + checkv + ". 인증코드번호의 기간경과를 피면하기 위하여 즉시인증코드를 입력하여 비밀번호를 수정하여주세요";
            t_Member model = DALM.Member_Select_Name(username, utype);
            if (model == null)
            {
                context.Response.Write("0;;;;;이 멤버는 찾을 수 없습니다！");
                return;
            }
            #region 手机找回
            if (mode == "1")
            {
                ...
                {
                    context.Response.Write("0;;;;;바인딩되지 않은 전화 번호！");
                    return;
                }
                bool TorF = false;
                BLLMS.Message_Add(model.nvc_MobilePhone, scontent, "비밀번호는 인증 코드를 전송 분실", model.i_Id, ref TorF);
                if (TorF)
                {
                    context.Response.Write("1;;;;;인증 코드가 휴대폰으로 전송 된" + model.nvc_MobilePhone.MobilePhoneReplaceX() + "，문자 메시지를받을 경우, 그 메시지 확인을 양해 해 주시기 바랍니다, 당신은 60 초 후에 다시 보낼 수 있습니다！");
                    return;
                }
                else
                {
                    context.Response.Write("0;;;;;확인 코드를 보내지 못했습니다! 암호를 변경하는 고객 서비스에 문의하시기 바랍니다！");
                    return;
                }
            }
            #endregion
            ...
            if (mode == "2")
            {
                ...
                {
                    context.Response.Write("0;;;;;바인딩되지 않은 메일！");
                    return;
                }
                bool TorF = false;
                BLLE.Email_Add(model.nvc_Email, "비밀번호 뒤찾는 인증코드 메일", scontent2, model.i_Id, ref TorF, "kr");
                if (TorF)
                {
                    context.Response.Write("1;;;;;이증코드메일이 성공적으로 보냈습니다 " + model.nvc_Email.EmailReplaceX() + " 메일 받지못했는 경우에 고객님이 60초후 다시보내드릴수있습니다！");
                    return;
                }
                else
                {
                    context.Response.Write("0;;;;;전자 메일을 보내지 못했습니다! 암호를 변경하는 고객 서비스에 문의하시기 바랍니다！");
                    return;
                }
            }
            #endregion
        ...
        {
            context.Response.Write("0;;;;;인증 코드가 만료되었습니다！");
        }
    }
    ...
    public void AddFaq(HttpContext context)
    {
        ...
        {
            context.Response.Write("유효한 이메일 주소를 입력 해주십시오");
            return;
        }
        if (StringHelper.CheckSqlKeyWord(subTitle) || StringHelper.CheckSqlKeyWord(conten))
        {
            context.Response.Write("당신이 사양에 맞지 않는 입력 문자를 확인하고 다시 입력 해주세요！");
            return;
        }
        ...
        {
            context.Response.Write("500 문자를 초과 할 수 없습니다！");
            return;
        }
        ...
        if (DALOM.OnlineMessage_Add(model) > 0)
            ...
            context.Response.Write("메시지 전송이 실패");
    ...
    public void sbjflogin(HttpContext context)
    {
        ...
        {
            context.Response.Write("일단 등록 상표 시스템");
        }
        else
        {
            ...
            if (member != null)
            {
                if (type == "1")//直接登录
                {
                    ...
                    if (member.i_IntegralMobileId != 0)
                    {
                        ...
                        if (m_im != null)
                        {
                            ...
                            {
                                context.Response.Write("아이디 또는 미빌번호가 틀렸습니다"); return;
                            }
                            #endregion
                        }
                        else
                        {
                            context.Response.Write("설정된 적분계좌 없습니다"); return;
                        }
                    }
                    else
                    {
                ...
                if (type == "2")
                {
                    ...
                    if (m_im != null)//绑定已有帐号
                    {
                        ...
                        {
                            context.Response.Write("이미 존재한계좌입니다  핸드폰번호또는 비밀번호가 틀렸습니다");
                        }
                        #endregion
                    }
                    else//开通新帐号
    ...
    public void zljflogin(HttpContext context)
    {
        ...
        {
            context.Response.Write("일단 등록 특허 시스템");
        }
        else
        {
