                ...
                if (fileExt == ".xlsx" || fileExt == ".xls")
                {
                    ...
                    if (cells.MaxDataRow >= 8)
                    {
                        ...
                        for (int i = 8; i < cells.MaxDataRow + 1; i++)
                        {
                            ...
                            {
                                if (DALP.Patent_Select_Number(cells[i, 2].StringValue, DALN.Nationality_Select_Name(cells[i, 4].StringValue, "cn").i_Id, uId, 0) <=0)
                                {
                                    #region
                        ...
                        div_a.InnerHtml = "<script>alert('成功导入" + ok + "个！有" + no + "个专利编号重复,重复的专利编号有：" + nostr + "');location.href='User_PatentAddOk.aspx?AddType=成功导入专利&AddOkCount=" + ok + "';</script>";
                    }
                    else
                    {
