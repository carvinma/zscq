<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Shop_M_SubIntegralProductType.aspx.cs" Inherits="Shop_Manage_Shop_M_Patent" %>
...
<body>
    ...
    <div>
        <h1>
            <%if (pageadd)
              { %><span class="action-span"><a href="Shop_A_SubIntegralProductType.aspx" target="main">添加商品小分类</a></span><%} %> <span class="action-span1">
                ...
                <asp:Repeater ID="reptlist" runat="server" >
                    <ItemTemplate>
                        <tr>
                            ...
                            <td align="center"><%# HanderParentName(Eval("i_parentid").ToString())%></td>
                            <td height="26" align="center">
                            <%if (pageupdate){ %><a <%# Eval("nvc_ChinaName") != null ? " style=\"color:" + Eval("nvc_ChinaName").ToString() + "\"" : ""%> href="Shop_A_SubIntegralProductType.aspx?id=<%#Eval("i_Id") %>&ye=<%=ye %>&Keyword=<%=Keyword %>"><%#Eval("nvc_ChinaName")%></a><%}else { %><%#Eval("nvc_ChinaName")%><%} %>
                            </td>
