<%@ Page Language="C#" AutoEventWireup="true" EnableEventValidation="false" CodeFile="Shop_M_CouponRebateRules.aspx.cs" Inherits="ihome_manage_shop_yhjclass" %>
...
<body>
    ...
    <h1>
        <%if (pageadd)
          ...
          {%><span class="action-span"><a href="Shop_A_CouponRebateRules.aspx">添加优惠券购返规则</a></span><%} %><span class="action-span1"><a href="Shop_Index_Main.aspx">管理中心</a> </span><span
            id="search_id" class="action-span1">- 优惠券购返规则列表 </span>
    ...
    <div class="list-div" id="listDiv">
            ...
            <asp:Repeater ID="Repeater1" runat="server" 
                ...
                <ItemTemplate>
                    <tr>
                        ...
                        <td height="25"><%# Set_BrandOrCategory(Eval("nvc_SId"), Eval("i_Type"))%></td>
                        <td height="25" align="center"><%# Eval("i_Where")%></td>
                        <td height="25" align="center"><asp:Button ID="Isbuy" runat="server" CommandName='Isbuy' CommandArgument='<%# Eval("i_Id") %>' CssClass='<%#Eval("i_State").ToString() == "1" ? "inputicoyesbutton" : "inputiconobutton"%>' />
                        </td>
                        <td height="25"><%# Set_CouponType(Eval("nvc_CouponType"), Eval("nvc_GiveNumber"))%></td>
                        <td height="25" align="center">
                            <%if (pageupdate){%>
                                <input type="button" title="修改" class="inputicoeditbutton" onclick="onUrlTo('<%# Eval("i_Id")%>','<%=ye %> ')"/>
                               <%-- <a class="inputicoeditbutton" href="Shop_A_CouponRebateRules.aspx?tid=<%# Eval("i_Id")%>&ye=<%=ye %>">修改</a>--%>
                            <%}%>
                        </td>
                    </tr>
                </ItemTemplate>
        ...
        <%if (pagedel)
          ...
          {%><table cellspacing='1' cellpadding='3'>
            <tr>
                <td><input name="checkall" id="checkall" type="checkbox" onclick="SelAll()" /><input type="button" id="Button2"
                 runat="server" value="删除" onclick="xwshow()" class="button" /></td>
            </tr>
        </table><%}%>
    </div>
...
function onUrlTo(i_Id, ye) {
    //alert(i_Id+"---"+ye);
    var url = "Shop_A_CouponRebateRules.aspx?tid=" + i_Id + "&ye=" + ye;
    window.location = url;
}
</script>