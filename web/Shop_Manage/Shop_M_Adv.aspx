<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Shop_M_Adv.aspx.cs" Inherits="Shop_Manage_Advert_MangeAdvert" %>
...
<body>
  ...
  <h1>
    <%if (pageadd)
      {%><span class="action-span"><a href="Shop_A_Adv.aspx?lang=<%=ddl_language.SelectedValue %>">添加广告</a></span><%}%>
    <span class="action-span1"><a href="Shop_Index_Main.aspx">管理中心</a> </span><span id="search_id" class="action-span1">- 广告管理 </span>
  ...
  <div class="list-div" id="listDiv">
    ...
    <%}%>
    <table cellspacing='1' cellpadding='3'>
      <asp:Repeater ID="Rpt_AdvType" runat="server" OnItemDataBound="advList_ItemDataBound">
        <ItemTemplate>
          <tr>
            <td colspan="4"><a href="###" onclick="showtr('<%# Eval("i_Id") %>');"><img id="img<%# Eval("i_Id") %>" src="images/tree_folder4.gif" style="border: 0;" /><b><%# Eval("nvc_Name") %><asp:Label ID="Lb_Items" runat="server"></asp:Label></b></a></td>
            <td width="10%" align="center">
              <%if (pageadd)
                {%><a href="Shop_A_Adv.aspx?i_typeid=<%#Eval("i_Id") %>&lang=<%=ddl_language.SelectedValue %>">添加广告</a><%} %><asp:HiddenField ID="Hi_Id" runat="server" Value='<%# Eval("i_Id") %>' />
            </td>
          </tr>
          ...
          <asp:Repeater ID="advList" runat="server">
            <ItemTemplate>
              <tr id="tr<%# Eval("i_TypeId") %>" style="display: none;">
                <td align="center">
                  <input name="selectid" onclick="Selbox(this)" type="checkbox" id="selectid" value="<%#Eval("i_Id") %>"></td>
                <td align="center">
                  <%# Set_Adv(Eval("i_Id"))%></td>
                <td align="center">
                  <%# Set_OpenWay(Eval("nvc_OpenWay"))%></td>
                <td align="center">
                  <%#Eval("i_Orderby")%></td>
                <td align="center">
                  <%if (pageupdate)
                    {%><a href="Shop_A_Adv.aspx?tid=<%#Eval("i_Id") %>&pagetype=showtr&trid=<%# Eval("i_TypeId") %>&lang=<%=ddl_language.SelectedValue %>">修改</a><%}%></td>
              </tr>
            </ItemTemplate>
          </asp:Repeater>
        </ItemTemplate>
