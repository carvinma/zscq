<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Shop_M_PatentType.aspx.cs" Inherits="Shop_Manage_Shop_M_Patent" %>
...
<body>
    ...
    <div>
        <h1>
            <%if (pageadd)
              { %><span class="action-span"><a href="Shop_A_PatentType.aspx">添加专利类型</a></span><%} %> <span class="action-span1">
        ...
        <div class="form-div">
        <asp:DropDownList ID="Drp_PGuoJiaType" runat="server">
        </asp:DropDownList>&nbsp;
        ...
        <div class="list-div" id="listDiv">
            <table cellspacing='1' cellpadding='3'>
                <tr>
                    ...
                    <th width="10%">国家</th>
                    <th width="10%">有效期</th>
                    <th width="13%">有效期起算日</th>
                    <th width="15%">缴费第一年起算日</th>
                </tr>
                <asp:Repeater ID="reptlist" runat="server" >
                    <ItemTemplate>
                        <tr>
                            ...
                            <%if (pageupdate){ %><a <%# Eval("nvc_Name") != null ? " style=\"color:" + Eval("nvc_Name").ToString() + "\"" : ""%> href="Shop_A_PatentType.aspx?id=<%#Eval("i_Id") %>&ye=<%=ye %>&Keyword=<%=Keyword %>&PGuoJiaType=<%= Drp_PGuoJiaType.SelectedValue %>"><%#Eval("nvc_Name")%></a><%}else { %><%#Eval("nvc_Name")%><%} %>
                            </td>
                            <td align="center"><%#Eval("nvc_EnglishName")%></td>
                            <td align="center"><%#Eval("nvc_JapaneseName")%></td>
                            <td align="center"><%#Eval("nvc_koreanName")%></td>
                            <td align="center"><%#Eval("nvc_ChinaName")%></td>
                            <td align="center"><%#Eval("i_YouXiaoQi")%></td>
                            <td align="center"><%#DALPT.GetStartDay(Convert.ToInt32(Eval("i_YouXiaoQiStart")))%></td>
                            <td align="center"><%#DALPT.GetStartDay(Convert.ToInt32(Eval("i_paymentFirstYear")))%></td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
            </table>
