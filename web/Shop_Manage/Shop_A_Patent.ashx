        ...
        if (context.Request["flag"] != null && context.Request["flag"].ToString() != "")
        {
            ...
            switch (flagtype)
            {
                ...
                case "patentfee":
                    ...
                    dal_PatentFee DALP = new dal_PatentFee();
                    for (int i = 0; i < SetFee.Split('-').Length - 1; i++)
                    {
                        int ID = Convert.ToInt32(HFID.Split('-')[i]);
                        t_PatentFee modelfee = DALP.PatentFee_Select_Id(ID);
                        ...
                        DALP.PatentFee_Update(modelfee);
                    }
                    context.Response.Write("ok");
                    break;
                #endregion
                ...
                case "yyt":
                    ...
                    {
                        yx_year = r1.i_YouXiaoQi.ToString();
                ...
                case "getpinfo":
                    ...
                    int pId = int.Parse(context.Request["patentTypeId"].ToString());//授权国国籍
                    ...
                    if (yijiaoyear != max_year)
                    {
                        dal_Patent DALPt = new dal_Patent();
                        status = DALPt.GetStatusByAnnualFeeRestDay(Convert.ToInt32(jiaofeeday));
                    }

                    string json = string.Format("{{ Year:'{0}', SetFee:'{1}',ShouXuFee:'{2}',AgentServiceFee:'{3}',MoneyType:'{4}',TotalFee:'{5}',ShengYear:'{6}',YearFeeTotal:'{7}',ChaoXiangFee:'{8}',YouXiaoQiStart:'{9}',YouXiaoQiEnd:'{10}',PrevJiaoFeeDate:'{11}',JiaoFeeDate:'{12}',JiaoFeeday:'{13}',DaiJiaoYear:'{14}',Status:'{15}'}}", nianxian, setfee, shouxufee, agentservicefee, moneytype, totalfee, shengnianxian, yearfeetotal, chaoxiangfee, youxiaoqistart, youxiaoqiend, prevjiaofeedate, jiaofeedate, jiaofeeday, daijiaoyear, status);
