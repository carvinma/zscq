<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Shop_M_MainIntegralProductType.aspx.cs" Inherits="Shop_Manage_Shop_M_Patent" %>
...
<body>
    ...
    <div>
        <h1>
            <%if (pageadd)
              { %><span class="action-span"><a href="Shop_A_MainIntegralProductType.aspx" target="main">添加商品大分类</a></span><%} %> <span class="action-span1">
                ...
                <asp:Repeater ID="reptlist" runat="server" >
                    <ItemTemplate>
                        <tr>
                            ...
                            <%if (pageupdate){ %><a <%# Eval("nvc_ChinaName") != null ? " style=\"color:" + Eval("nvc_ChinaName").ToString() + "\"" : ""%> href="Shop_A_MainIntegralProductType.aspx?id=<%#Eval("i_Id") %>&ye=<%=ye %>&Keyword=<%=Keyword %>"><%#Eval("nvc_ChinaName")%></a><%}else { %><%#Eval("nvc_ChinaName")%><%} %>
                            </td>
