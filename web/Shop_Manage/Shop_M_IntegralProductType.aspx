<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Shop_M_IntegralProductType.aspx.cs" Inherits="Shop_Manage_Shop_M_Patent" %>
...
<body>
    ...
    <div>
        <h1>
            <%if (pageadd)
              { %><span class="action-span"><a href="Shop_A_PatentType.aspx">添加商品分类</a></span><%} %> <span class="action-span1">
                ...
                <asp:Repeater ID="reptlist" runat="server" >
                    <ItemTemplate>
                        <tr>
                            ...
                            <%if (pageupdate){ %><a <%# Eval("nvc_ChinaName") != null ? " style=\"color:" + Eval("nvc_ChinaName").ToString() + "\"" : ""%> href="Shop_A_PatentType.aspx?id=<%#Eval("i_Id") %>&ye=<%=ye %>&Keyword=<%=Keyword %>&PGuoJiaType=<%= Drp_PGuoJiaType.SelectedValue %>"><%#Eval("nvc_Name")%></a><%}else { %><%#Eval("nvc_Name")%><%} %>
                            </td>
