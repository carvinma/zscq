...
function Calendar(beginYear, endYear, language, patternDelimiter, date2StringPattern, string2DatePattern) {
	...
	this.colors = {"bg_cur_day":"#ff8600","bg_over":"#ecf0f3","bg_out":"#8fcfff"}
};
...
Calendar.prototype.bindData1 = function (id) {
	var calendar = this;
		...
		if (dateArray[i]){
			tds[i].onclick = function () {
				if (calendar.dateControl){
					var aa =new Date(calendar.date.getFullYear(),
							calendar.date.getMonth(),
                      this.innerHTML).format(calendar.date2StringPattern);
                      calendar.dateControl.value=aa;
				     if(id==1)
                       {
                     var yr= document.getElementById("txt_AnnualFeeYear").value;
                    var bb =new Date(calendar.date.getFullYear()+parseInt(yr),
							calendar.date.getMonth(),
                      this.innerHTML).format(calendar.date2StringPattern);
                         document.getElementById("txt_AnnualFeeDateEnd").value=bb;
                         }			
                    if(id==2)
                       {
                        var prevYear =new Date(calendar.date.getFullYear(),calendar.date.getMonth(),
                        this.innerHTML).format(calendar.date2StringPattern);
                        var NextYear =new Date(calendar.date.getFullYear()+1,calendar.date.getMonth(),
                        this.innerHTML).format(calendar.date2StringPattern);
                        document.getElementById("txt_AnnualFeeNextYear").value=NextYear;
                        var s1 = prevYear.replace(/-/g, "/");
                        var myDate = new Date();
                        var s2 = NextYear.replace(/-/g, "/");
                        d1 = new Date(s1);
                        d2 = new Date(s2);
                        var time = d2.getTime() - d1.getTime();
                        var days = parseInt(time / (1000 * 60 * 60 * 24));
                        document.getElementById("txt_AnnualFeeRestDay").value=days;
                         }	
				}
				calendar.hide();
			}
...
Calendar.prototype.show1 = function (dateControl, popuControl) {
	if (this.panel.style.display == "block") {
		this.panel.style.display = "none";
	}
	if (!dateControl){
...
Calendar.prototype.show2 = function (dateControl, popuControl) {
	if (this.panel.style.display == "block") {
		this.panel.style.display = "none";
	}
	if (!dateControl){
...
var divs = [];
divs[divs.length] = '<div id="__calendarPanel" style="position:absolute;z-index:10000;display:none;background-color:#FFFFFF;border:1px solid #84a1c1;width:200px;height:210px;filter: progid:DXImageTransform.Microsoft.Shadow(color=#b0bcc8,direction=135,strength=3);">';
...
cis[cis.length] ='th.theader {font-weight:normal;background-color:#60a3d5;color:#FFFFFF;width:24px;}';
cis[cis.length] ='select.year {width:64px;}';
cis[cis.length] ='select.month {width:60px;}';
cis[cis.length] ='td {font-size:12px;text-align:center;cursor:pointer;}';
...
cis[cis.length] ='input.l {border: 1px solid #CCCCCC;background-color:#EFEFEF;width:20px;height:20px;cursor:pointer;}';
cis[cis.length] ='input.r {border: 1px solid #CCCCCC;background-color:#EFEFEF;width:20px;height:20px;cursor:pointer;}';
cis[cis.length] ='input.b {border: 1px solid #FFFFFF;padding-top: 2px;width:100%;height:20px;background-image: url(images/button_cl.gif);font-size: 12px;color: #FFFFFF;cursor:pointer;}';
cis[cis.length] ='-->';
