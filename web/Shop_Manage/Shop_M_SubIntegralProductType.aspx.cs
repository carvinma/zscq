   ...
   public void Bind_Rpt_ProductType(int index)
    {
        ...
        int pagesiz = 20;
        int count = 0;
        reptlist.DataSource = DALPT.SubIntegralProductType_SelectPage(pageindex, pagesiz, ref count);
        reptlist.DataBind();
    ...
    protected void btnDel_Click(object sender, EventArgs e)
    {
        if (Request.Form["inputPageid"] != null)
        {
            ...
            Manager.AddLog(0, "商品小分类管理", "删除商品小分类");
        }
        Bind_Rpt_ProductType(aspPage.CurrentPageIndex);
        HiddenDel.Value = "del";
    }
    public string HanderParentName(string parentid)
    {
        int i_parentid = int.Parse(parentid);
        string pname=DALPT.IntegralProductType_Select_Id(i_parentid).nvc_ChinaName;
