<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Shop_M_Mail.aspx.cs" Inherits="Shop_Manage_shop_mailcount" %>
...
<body>
  ...
  <div>
    <h1>
      <span class="action-span1"><a href="Shop_Index_Main.aspx">管理中心</a> </span><span id="search_id" class="action-span1">- 发送邮件列表</span>
    ...
    <div class="form-div">
      用户名：<asp:TextBox ID="txtKeyword" runat="server" CssClass="input"></asp:TextBox>&nbsp;<asp:DropDownList ID="ddlType" runat="server">
        <asp:ListItem>发送成功</asp:ListItem>
        <asp:ListItem>发送失败</asp:ListItem>
        <asp:ListItem Selected="True">所有状态</asp:ListItem>
      </asp:DropDownList>
      &nbsp;语言版本：
      <asp:DropDownList ID="Drp_LanguageList" runat="server">
      </asp:DropDownList>
      <asp:Button ID="Button1" runat="server" Text="搜索" class="button" OnClick="btnSerach_Click" />
    </div>
        ...
        <asp:Repeater ID="rpList" runat="server">
          ...
          <ItemTemplate>
            <tr>
              ...
              <td align="center">
                <%if (pageadd)
                  { %><a href="Shop_Apl_Mail.aspx?id=<%#Eval("i_Id")%>" title='点击进入发送页面，重新发送此邮件'><%#Eval("nvc_Title")%></a><%}
                  else
                  { %><%#Eval("nvc_Title")%><%} %>
              </td>
              <td align="center">
