            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null)
                    {
                        ...
                        sb_trademrk.Append("<td  align='center' bgcolor='#FFFFFF'>" + (model.i_IsPayState.ToString() == "2" ? "Have fee, wait for the next payment" : (model.i_IsPayState.ToString() == "3" ? "Invalid" : "Unpaid")) + "</td>");
                        ...
                        if (tm != null)
                        {
                            ...
                            {
                               sb_trademrk.Append(" <a href=\"user_World_sbweituo.aspx?id=" + tid + "&type=2&zixing=ok\" target=\"_blank\" class=\"ac5t\">Attorney Download</a><br>");

                            }
                        }
                        sb_trademrk.Append("<a href=' user_World_sb.aspx?id=" + tid + "&zixing=ok' class='ac5t'>Application download</a></td>");
