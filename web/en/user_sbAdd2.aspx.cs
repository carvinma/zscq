                    ...
                    { 
                        for (int i = 6; i < cells.MaxDataRow + 1; i++)
                        {
                            ...
                            if (td == null)
                            {
                                ...
                                {
                                    info = "At row " + i + "time format entered incorrectly, so please stop inputting! Input " + oknum + "data successfully！";
                                }
                                
                                model.i_MiaoShuType = 1;
                                if (miaoshu.IndexOf("Word") >= 0)
                                {
                                    model.i_MiaoShuType = 1;
                                }
                                if (miaoshu.IndexOf("Device") >= 0)
                                {
                                    model.i_MiaoShuType = 2;
                                }
                                if (miaoshu.IndexOf("and") > 0)
                                {
                                    model.i_MiaoShuType = 3;
                                }
                                model.i_JiaoFeiType = 2;//默认是Entrust to renew
                        ...
                        div_a.InnerHtml = "<script>alert('Input " + oknum + " successfully！" + info + "" + sbnum + " Trademark numbers are duplicate,which are：" + fstr + "');location.href='user_sbAddOk.aspx?shu=" + oknum + "&ok=success';</script>";//  Response.Redirect("user_sbAddOk.aspx?shu=" + oknum + "&ok=success");
                        //Input 1 successfully! 2 patent numbers are duplicate, which are
                    }
                    else
                    {
