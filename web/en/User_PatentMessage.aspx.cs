...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    void Add_OnlineMsg()
    {
        ...
        if (DALOM.OnlineMessage_Add(model) > 0)
            div_a.InnerHtml = "<script>alert(\"The question was already sent to customer service center. Accordingly, we will make response to you within 24 hours, so please login in and check our response in due time.\");window.location='user_patentmessage.aspx'</script>";
        else
            div_a.InnerHtml = "<script>alert(\"Fail to send the leaving message！\");</script>";
    }
