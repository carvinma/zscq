...
public partial class user_shopddck : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>Order detailed information_My order of gift exchange_System of Reward Points Shop_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    public void Bind_Page_Value()//绑定所有Address
    {
        if (Request.QueryString["orderid"] != null && Request.QueryString["orderid"].ToString() != "")
        {
            ...
            if (model != null && model.i_MemberId.ToString() == Hi_MemberId.Value)
            {
                ...
                Address += string.IsNullOrWhiteSpace(model.nvc_EnName) == true ? "" : model.nvc_EnName + "&nbsp;&nbsp;";
                ...
                OrderStatus = DALIO.Set_IntegralOrderState(model.i_State,"en");
