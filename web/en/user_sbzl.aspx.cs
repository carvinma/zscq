...
public partial class user_sbzl : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>Modify information_My information center_System of Trademark Maintenance_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    public void Bind_Page_Info()//绑定Basic information
    {
        ...
        if (model != null)
        {
            ...
            {
                zhutiwenjian = "<a href='../" + model.nvc_ZhuTiFile + "' target='_blank' title='click to check'>Existed(click to check)</a><br><span  style=\"color:Red;\">For any change on Certificate documents, please upload again.</span>";
            }
        }
    }
    ...
    protected void Bt_Submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (result.Count() > 0)
            {
                div_a.InnerHtml = "<script>alert('E-mail is already in use！');</script>";
                return;
            }
            ...
            if (result2.Count() > 0)
            {
                div_a.InnerHtml = "<script>alert('Mobile phone number is already in use！');</script>";
                return;
            }
        }
            if (hi_guoji.Value == "1")
            ...
            if (FileUpload3.HasFile)
            {
                ...
                if (fileNameExt == ".jpg" || fileNameExt == ".gif" || fileNameExt == ".png" || fileNameExt == ".pdf")
                {
                    ...
                    {
                        MessageBox.Show(this.Page, "The size of document cannot be bigger than 500KB!");
                        // div_a.InnerHtml = "<script>alert('文件大小不能超过 512KB');</script>";
                        return;
                    }
                    string toFileName = DateTime.Now.ToString("HHmmssff") + new Random().Next(100);
                    string path = System.AppDomain.CurrentDomain.BaseDirectory + "File_Zscq/File_ShangBiao/" + toFileName + "Proof of subject qualification" + fileNameExt;
                    FileUpload3.SaveAs(path);
                    if (model.nvc_ZhuTiFile != null)
                    {
                        DelPhoto.DeleteFile(model.nvc_ZhuTiFile);
                    }
                    model.nvc_ZhuTiFile = "File_Zscq/File_ShangBiao/" + toFileName + "Proof of subject qualification" + fileNameExt;
                }
                else
                {
                    MessageBox.ShowAndRedirect(this.Page,"File format error！","user_sbzl.aspx");
                    return;
                }
            }
            DALM.Member_Update(model);

            //#region 添加到我的Address薄
            ...
            // div_a.InnerHtml = "<script>alert('Modify successfully！');location.href='user_sbzl.aspx';</script>";
            ScriptManager.RegisterStartupScript(UpdatePanel2, this.GetType(), "", "<script>alert('Modify successfully！');location.href='user_sbzl.aspx';</script>", false);
        }
        catch
        {
            ScriptManager.RegisterStartupScript(UpdatePanel2, this.GetType(), "", "<script>alert('Parameter error, please check after the submission！');location.href='user_sbzl.aspx';</script>", false);
            //div_a.InnerHtml = "<script>alert('Parameter error, please check after the submission！');location.href='user_sbzl.aspx';</script>";
        }
    }
