...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (Request.Cookies["hqht_Trademarktidstr"] != null && Request.Cookies["hqht_Trademarktidstr"].Value != "")
        {
                ...
                if (flag != null && flag != "")
                {
                    ...
                    for (int j = 0; j < flaglist.Length; j++)
                    {
                        if (flaglist[j] != "" || flaglist[j] != null)
                        {
                            ...
                            {
                                yhqid += tc.i_Id + ",";
                                youhui += "+" + t.nvc_ENName + bizhong + t.i_Money;
            ...
            if (sbnum == 0)
            {
                div_a.InnerHtml = "<script>alert('Did not want to pay trademarks！');localtion.href='user_sblb.aspx';</script>";
            }
            else
            {
                if (emalladdress != "")
                {
                    ...
                    if (input_payway.Value == "线下汇款")
                    {
                        ...
                        for (int i = 0; i < OrderModer.nvc_BankId.Split(',').Length; i++)
                        {
                            ...
                            if (tb != null)
                            {
                                str1 += "<br/><font color='red'>Bank Details：" + tb.nvc_BankDetails + "<br/>SWIFT CODE：" + tb.nvc_SwiftCode + "<br/>Account Name：" + tb.nvc_AccountName + "&nbsp;<br/>Account Number：" + tb.nvc_BankNumber + "</font><br/>";
                            }
                        }
                        huikuanbankinfo = str1;
                    }

                    BLLE.Email_Add(emalladdress, "Trademark payment orders", "Hello! Your order number：" + OrderModer.nvc_OrderNumber + " <br/>  Order time：" + OrderModer.dt_AddTime + "  <br/>Your payment method is offline payment, and please check our bank account information as following:" + huikuanbankinfo + " <br/><br/> Detailed renewal fee of the selected Trademark:<br/>" + hi_feeinfo.Value + "<br/>Please pay within 24 working hours! For questions, please contact the Global Patent! <br/> Tel：86-10-84505596<br/>E-MAIL：pat-annuity@hqht-online.com", uId, ref states, "en");
                }
                ...
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null && muser != null)
                    {
                        if (muser.i_UserTypeId == 3)
                        {
                            ...
                            {
                                MessageBox.ShowAndRedirect(this.Page, "The scanned copy of Trademark certificate with respect to Trademark number "+model.nvc_SBRegNum+" has not been uploded yet, please click the button in Trademark list to upload！", "user_sblb.aspx");
                                return;
                            }
                        }
                        else if (muser.i_UserTypeId != 3)
                        {
                            ...
                            {
                                MessageBox.ShowAndRedirect(this.Page, model.nvc_SBRegNum + "The mark does not upload the qualification certificate,please click upload my information！", "user_sbzl.aspx");
                                return;
                            }
                        }
            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null)
                    {
                        ...
                        sb_trademrk.Append("<td align='center' bgcolor='#FFFFFF'>" + (model.i_JiaoFeiType.ToString() == "1" ? "Renew by oneself" : "Entrust to renew") + "</td>");
                        ...
                        //sb_trademrk.Append("<td width='77' align='center' bgcolor='#FFFFFF'>" + (model.i_State.ToString() == "1" ? "已审核" : (model.i_State.ToString() == "2" ? "未通过" : "Not checked")) + "</td>");
                       // sb_trademrk.Append("<td width='50' align='center' bgcolor='#FFFFFF'>" + (model.i_IsPayState.ToString() == "2" ? "Already pay,Waiting for the next payment" : (model.i_IsPayState.ToString() == "3" ? "Invalid" : "Unpaid")) + "</td>");
                        sb_trademrk.Append("<td align='center' bgcolor='#FFFFFF'>" + model.nvc_SbDaoqiTime + "</td>");
                        sb_trademrk.Append("<td align='center' bgcolor='#FFFFFF' class=\"hidesbinfo\"><a href='user_sbck.aspx?t_r_id=" + tid + "&href=" + href + "' class='ac5t'>Check</a> <a href=" + "'?pagetype=del&cs=" + tid + "," + number + "' class='ac5t'>Delete</a></td>");
                        sb_trademrk.Append("</tr>");
                    }
                }
            }
                ...
                if (mm.i_GuoJiId != null)
                {
                    ...
                    if (na != null)
                    {
                        ...
                        if (na.nvc_Name == "中国")
                        {
                            ...
                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">Official fee：</td><td width=\"110\">" + (model1.dm_TrademarkMoney).ToString("0.00") + "*" + sbnum + "</td><td width=\"30\"></td><td width=\"100\"></td></tr>");
                            if (zhinajinnum != 0)
                            {
                                Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">Surcharge：</td><td width=\"110\">" + (model1.dm_TMZhiNaJin).ToString("0.00") + "*" + zhinajinnum + "</td><td width=\"30\"></td><td width=\"100\"></td></tr>");
                            }
                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">Service fee：</td><td width=\"110\">" + (dalifee).ToString("0.00") + "*" + sbdailinum + "</td><td width=\"30\"></td><td width=\"100\"></td></tr>");

                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\"></td><td width=\"110\" align=\"right\">Subtotal：</td><td width=\"30\">CNY</td><td width=\"100\" align=\"left\">" + totalmoney + "</td></tr>");

                            Str_AllMoney.Append("<tr align=\"left\"><td width=\"200\" align=\"right\"></td><td width=\"110\" align=\"right\">Total fee：</td><td style=\"color:red;\" width=\"30\">CNY</td><td style='color:red;width:100px;' id='allmoney'  align=\"left\">" + totalmoney + "</td></tr>");
                        }
                        else
                        {
                            //Bind_Rpt_BankList(1,0);//国外
                            ...
                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">Official fee：</td><td width=\"110\">" + (model1.dm_TrademarkMoney).ToString("0.00") + "*" + sbnum + "</td><td width=\"30\"></td><td style=\"width:100px;\"></td></tr>");
                            if (zhinajinnum != 0)
                            {
                                Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">Surcharge：</td><td width=\"110\">" + (model1.dm_TMZhiNaJin).ToString("0.00") + "*" + zhinajinnum + "</td><td width=\"30\"></td><td style=\"width:100px;\"></td></tr>");
                            }
                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\">Service fee：</td><td width=\"110\">" + (dalifee).ToString("0.00") + "*" + sbdailinum + "</td><td width=\"30\"></td><td style=\"width:100px;\"></td></tr>");

                            Str_Money.Append(" <tr align=\"left\"><td width=\"200\" align=\"right\"></td><td width=\"110\" align=\"right\">Subtotal：</td><td width=\"30\">CNY</td><td style=\"width:100px;\" align=\"left\">" + totalmoney + "</td></tr>");

                            Str_AllMoney.Append("<tr align=\"left\"><td width=\"200\" align=\"right\"></td><td width=\"110\" align=\"right\">Total fee：</td><td style='color:red;' width=\"30\">CNY</td><td style='color:red;' width=\"100\" id='allmoney'  align=\"left\">" + totalmoney + "</td></tr>");
    ...
    {
        return name == "Courier delivery" ? "quhuo" : "";
    }
    ...
    void Bind_Page_PayWay()//绑定Payment methods
    {
    }
