...
function shoucang() {
  ...
  $.ajax({ type: "get", url: "Ajax_Handler.aspx", data: cs,
    success: function (data) {
      if (data == "1") {
        alert("Successful add to Favorites！");
      }
      else if (data == "2") {
        window.parent.location = 'Login.html?pageurl=ProductShow-' + obj('Hi_PId').value + '.html';
      }
      else if (data == "3") {
        alert("Favorites have the goods!");
      }
      else if (data == "4") {
        alert("The merchandise not found!");
      }
    }
  });
}
...
function commentallpage(vpage) {
  obj('Y_Foot_Page1').innerHTML = "<img alt=\"Please wait for a while\" src=\"images/loading.gif\" style=\"margin-left:350px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function zxpage(vpage) {
  obj('Y_Foot_Page2').innerHTML = "<img alt=\"Please wait for a while\" src=\"images/loading.gif\" style=\"margin-left:340px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function buynotepage(vpage) {
  obj('divbuynote').innerHTML = "<img alt=\"Please wait for a while\" src=\"images/loading.gif\" style=\"margin-left:340px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function commentsubmit() {
  if (obj("t_content").value == "Civilized Internet, Li Zhi speech。" || obj("t_content").value.toString() == "") {
    obj('t_content').focus();
...
function GetLogin() {
  if (obj("text_username").value == "用户名" || obj("text_username").value.toString() == "") {
    ...
    changecolor('div_username', 0, "Please enter username！");
    return false;
  }
  if (obj("TB_pwd").value == "") {
    ...
    obj('TB_pwd').focus();
    changecolor('div_pwd', 0, "Please enter password！");
    return false;
  }
  if (obj("text_v")) {
    if (obj("text_v").value == "验证码" || obj("text_v").value.toString() == "") {
      obj('text_v').focus();
      changecolor('div_checkcode', 0, "Please enter verification code！");
      return false;
    }
  }
...
function checkonsubmit() {
  if (!obj("cb_Agreement").checked) {
    alert("I'm sorry, to register as a member of this site, you must agree to the member registration protocol！");
    return false;
  }
  //    if (obj("text_regmail").value.toString() == "" || obj('text_regmail').value == "邮箱/手机") {
  //        obj("text_regmail").focus();
  //        changecolor('div_regmail', 0, "Please enter username！", 'li_tixing2');
  //        return false;
  //        }
  ...
  if (reg.test(obj("text_regmail").value.toString()) == false && reg3.test(obj('text_regmail').value) == false) {
    ...
    changecolor('div_regmail', 0, "Username format error！", 'li_tixing2');
    return false;
  }
  ...
  if (obj('Hi_Repeat').value == "1") {
    obj("text_regmail").select();
    changecolor('div_regmail', 0, obj("text_regmail").value + "Already exists！", 'li_tixing2');
    return false;
  }
  ...
  if (obj("pwd_regpwd").value.toString() == "") {
    ...
    changecolor('div_regpwd', 0, "Please enter password！");
    return false;
  }
  if (obj("pwd_regpwd").value.length < 6 || obj("pwd_regpwd").value.length > 21) {
    ...
    obj('text_regpwd').style.display = 'none';
    changecolor('div_regpwd', 0, "Password length is between 6 to 21！");
    return false;
  }
  if (obj("pwd_regpwdok").value.toString() == "") {
    ...
    changecolor('div_regpwdok', 0, "Please enter your password again！");
    return false;
  }
  if (obj("pwd_regpwd").value.toString() != obj("pwd_regpwdok").value.toString()) {
    ...
    obj('text_regpwdok').style.display = 'none';
    changecolor('div_regpwdok', 0, "Two different input password！");
    return false;
  }
  if (obj('text_regcheckcode').value == "" || obj('text_regcheckcode').value == "验证码") {
    obj("text_regcheckcode").focus();
    changecolor('div_regcheckcode', 0, "Please enter verification code！");
    return false;
  }
  obj('IBt_Register').innerHTML = "Is registered…";
}
...
function blurpnumber(wz, pid, colorx, v1, v2, v3) {
  ...
  if (parseInt(number.value) < 1) {
    alert('The quantity of the goods is at least1!');
...
//  }
  if (obj("text_gudingdianhua").value == "") {
    alert("Please enter the telephone numberephone!");
...
function updatepwd() {
  ...
  if (document.getElementById("text_pwd3").value.toString() != document.getElementById("text_pwd2").value.toString()) {
    alert("The new password two times entered are inconsistent!");
    document.getElementById("text_pwd2").focus();
    return false;
  }
