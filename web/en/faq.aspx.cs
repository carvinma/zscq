...
public partial class faq : System.Web.UI.Page
{
    ...
    {
        t_PageInfo model = new dal_PageInfo().PageInfo_Select_Id(22);
    ...
    void Bind_Rpt_List()
    {
        ...
        var iquery = from i in dvdc.vw_OnlineMessage where i.nvc_Language=="en" select i;
        if (!string.IsNullOrWhiteSpace(Request.QueryString["pagekey"]))
        {
            ...
            {
                lb_msg.Text = "The characters you entered are not in accordance with rules, so please reenter the characters after check！";
                return;
            }
    ...
    protected void ibtn_Add_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (DALOM.OnlineMessage_Add(model) > 0)
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.GetType(), "msg", "<script>alert(\"The question was already sent to customer service center. Accordingly, we will make response to you within 24 hours, so please login in and check our response in due time.\");window.location='faq.aspx';</script>", false);
        else
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.GetType(), "msg", "<script>alert(\"Fail to send the leaving message！\");</script>", false);

    }

    /// <summary>
    /// 给Online Q & A中的email框赋值
    /// </summary>
    void Bind_Email()
    {
        int uid = 0;
