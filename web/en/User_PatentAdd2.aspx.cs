...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    protected void ImgBtn_Add2_Click(object sender, ImageClickEventArgs e)
    {
                ...
                if (fileExt == ".xlsx" || fileExt == ".xls")
                {
                    ...
                    if (cells.MaxDataRow >= 8)
                    {
                        ...
                        for (int i = 8; i < cells.MaxDataRow + 1; i++)
                        {
                            ...
                            {
                                if (DALP.Patent_Select_Number(cells[i, 2].StringValue, DALN.Nationality_Select_Name(cells[i, 4].StringValue, "en").i_Id, uId, 0) <= 0)
                                {
                                    #region
                                    t_Patent model = new t_Patent();
                                    model.i_MemberId = uId;//会员编号*
                                    model.nvc_Number = cells[i, 2].StringValue;//Patent number*
                                    model.nvc_Name = cells[i, 3].StringValue;//Title of patent
                                    model.i_AuthorizeNationalityId = DALN.Nationality_Select_Name(cells[i, 4].StringValue,"en").i_Id;//Granted Country*
                                    model.i_PatentType = DALPT.PatentType_Select_GuoJiaIdAndName(model.i_AuthorizeNationalityId, cells[i, 5].StringValue, "en").i_Id;//Type of patent*
                                    ...
                                    {
                                        model.i_YaoQiuXiang = Convert.ToInt32(cells[i, 8].StringValue);//Number of claims
                                    }
                                    else
                                    {
                                        model.i_YaoQiuXiang = 0;
                                    }
                                    model.nvc_PatentHolder = cells[i, 9].StringValue;//Patentee
                                    if (cells[i, 10].StringValue != "")
                                    {
                                        model.i_PatentHolderNationalityId = DALN.Nationality_Select_Name(cells[i, 10].StringValue, "en").i_Id;//Nationality of patentee
                                    }
                                    model.nvc_PatentInventor = cells[i, 11].StringValue;//Inventor
                                    model.nvc_AnnualFeePaymentYear = cells[i, 12].StringValue;//专利已缴年度*
                                    if (cells[i, 13].StringValue != "")
                                    {
                                        model.dt_AnnualFeePrevYear = Convert.ToDateTime(cells[i, 13].StringValue);//Last deadline*
                                    }
                                    model.nvc_PriorityNumber = cells[i, 14].StringValue;//Priority application number
                                    if (cells[i, 15].StringValue != "")
                                    {
                                        model.i_PriorityNationalityId = DALN.Nationality_Select_Name(cells[i, 15].StringValue, "en").i_Id;//优先权申请国
                                    }
                                    if (cells[i, 16].StringValue != "")
                                    {
                                        model.dt_PriorityDate = Convert.ToDateTime(cells[i, 16].StringValue);//Priority filing date
                                    }
                                    ...
                                    model.i_AnnualFeeYear = DALPT.PatentType_Select_GuoJiaIdAndName(model.i_AuthorizeNationalityId, cells[i, 5].StringValue,"en").i_YouXiaoQi;//有效维持年限
                        ...
                        UserLog.AddUserLog(Request.Cookies["hqhtshop"]["hqht_zl_uid"], "专利系统", "批量上传专利");
                        div_a.InnerHtml = "<script>alert('Input " + ok + " successfully！" + no + " patent numbers are duplicate,which are：" + nostr + "');location.href='User_PatentAddOk.aspx?AddType=Quantity of patent successfully inputted&AddOkCount=" + ok + "';</script>";
                    }
                    else
                    {
                        div_a.InnerHtml = "<script>alert('No data')</script>";
                    }

                }
                else
                {
                    div_a.InnerHtml = "<script>alert('Please upload Excel form！')</script>";
                }
            }
        }
        catch (Exception ex)
        {
            ...
            //批量导入专利信息时，出现“data abnormal”（数据异常），这句英文提示语应改为“Abnormal data!”				
            div_a.InnerHtml = "<script>alert('Abnormal data!')</script>";
        }
    }
}