...
<body>
  ...
  <div id="buy_edit_persondata" class="buy_data" style="display: block;">
      ...
      <ItemTemplate>
        <dl>
          <dt style="text-align: center;">
            <input type="radio" name="address" onclick="changeaddress('<%# Eval("i_Id")%>','<%# Set_AddressName(Eval("i_PId"), Eval("i_CId"), Eval("i_AId"))%>&nbsp;&nbsp;<%# Eval("nvc_StreetAddress")%>&nbsp;&nbsp;（<%# Eval("nvc_Consignee")%>&nbsp;&nbsp;收）&nbsp;&nbsp;<%# Eval("nvc_MobilePhone")%>')" <%# Hi_AddressId.Value != Eval("i_Id").ToString() ? "" : " checked=\"checked\""%> /></dt>
          <dd>
            <%# Set_AddressName(Eval("i_PId"), Eval("i_CId"), Eval("i_AId"))%>&nbsp;&nbsp;<%# Eval("nvc_StreetAddress")%>&nbsp;&nbsp;（<%# Eval("nvc_Consignee")%>&nbsp;&nbsp;收）&nbsp;&nbsp;<%# Eval("nvc_TelPhone")%>&nbsp;&nbsp;<%# Hi_Mr.Value != Eval("i_Id").ToString() ? "<a href=\"?aid=" + Hi_AddressId.Value + "&PageType=mr&mid=" + Eval("i_Id") + "&ctype=" + Hi_CType.Value + "\">[设为默认]</a>" : "<span style=\"color:#e3441c;\">默认地址</span>"%>&nbsp;&nbsp;<a href="?aid=<%= Hi_AddressId.Value %>&PageType=del&did=<%# Eval("i_Id")%>&ctype=<%= Hi_CType.Value %>" onclick="javascript:if(!window.confirm('确认删除？')){return false;}">[Delete]</a></dd>
        </dl>
      </ItemTemplate>
    </asp:Repeater>
    <dl>
    ...
    <dl>
      <dt><span class="red">*</span>收货人地区：</dt>
      <dd>
