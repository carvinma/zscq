        ...
        if (Hi_AddressId.Value == "0")
        {
            switch (DALRA.ReceiveAddress_Add(model))
            {
                ...
                case 1:
                    ...
                    div_a.InnerHtml = "<script>alert('Add successfully！'); window.location='User_PatentAddress.aspx'</script>";
                    Response.Redirect("User_PatentAddress.aspx");
                    break;
                case 2:
        ...
        else
        {
            switch (DALRA.ReceiveAddress_Update(model))
            {
                ...
                case 1:
                    ...
                    div_a.InnerHtml = "<script>alert('Modify successfully！'); window.location='User_PatentAddress.aspx'</script>";
                    Response.Redirect("User_PatentAddress.aspx");
                    break;
                case 2:
                    div_a.InnerHtml = "<script>alert('You have the same recipient and the same address information already, which is no need to add again!');</script>";
