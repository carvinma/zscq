...
public partial class user_shopdzadd : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>Add Address_My information center_System of Reward Points Shop_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    protected void Bt_AddAddress_Click(object sender, EventArgs e)
    {
        ...
        //{
        //    div_a.InnerHtml = "<script>alert('请输入正确的Mobile phone number!');</script>"; return;
        //}
        if (text_shouhuoren.Value == "")
        {
            div_a.InnerHtml = "<script>alert('Please input the consignee!');</script>"; return;
        }
        if (this.ddl_guojia.SelectedValue == "0" )
        {
            div_a.InnerHtml = "<script>alert('Please select a country!');</script>"; return;
        }
        ...
        if (Hi_AddressId.Value == "0")
        {
            switch (DALRA.ReceiveAddress_Add(model))
            {
                ...
                case 1:
                    ...
                    div_a.InnerHtml = "<script>alert('Add successfully！'); window.location='user_shopdz.aspx'</script>";
                    break;
                case 2:
        ...
        else
        {
            switch (DALRA.ReceiveAddress_Update(model))
            {
                ...
                case 1:
                    ...
                    div_a.InnerHtml = "<script>alert('Modify successfully！'); window.location='user_shopdz.aspx'</script>";
                    break;
                case 2:
                    div_a.InnerHtml = "<script>alert('You have the same recipient and the same address information already, which is no need to add again!');</script>";
                    break;
