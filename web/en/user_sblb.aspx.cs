...
using zscq.DAL;
using zscq.Model;
public partial class user_sblb : System.Web.UI.Page
{
    ...
    public void Bind_Page_value()
    {
        ...
        {
            jiaofeistate = int.Parse(Request.QueryString["jiaofeistate"].ToString()); //统计page面传递过来的値 缴费状态    
        }
        if (Request.QueryString["isjiaofei"] != null && Request.QueryString["isjiaofei"] != "")
        {
            isjiaofei = int.Parse(Request.QueryString["isjiaofei"].ToString()); //统计page面传递过来的値 是否缴费     
        }
        if (Request.QueryString["sb_num"] != null && Request.QueryString["sb_num"] != "")
        {
            sb_num = Request.QueryString["sb_num"].ToString(); //Searchpage面传递过来的値
        }
        if (Request.QueryString["sb_type"] != null && Request.QueryString["sb_type"] != "")
        {
            sb_type = Request.QueryString["sb_type"].ToString(); //Searchpage面传递过来的値 
        }
        if (Request.QueryString["sb_regname"] != null && Request.QueryString["sb_regname"] != "")
        {
            sb_regname = Request.QueryString["sb_regname"].ToString(); //Searchpage面传递过来的値    
        }
        if (Request.QueryString["sb_regguoji"] != null && Request.QueryString["sb_regguoji"] != "")
        {
            sb_guoji = Request.QueryString["sb_regguoji"].ToString(); //Searchpage面传递过来的値 
        }
        if (Request.QueryString["sb_passtime"] != null && Request.QueryString["sb_passtime"] != "")
        {
            sb_passtime = Request.QueryString["sb_passtime"].ToString(); //Searchpage面传递过来的値
        }
        if (Request.QueryString["c_anjuanhao"] != null && Request.QueryString["c_anjuanhao"] != "")
        {
            c_anjuanhao = Request.QueryString["c_anjuanhao"].ToString(); //Searchpage面传递过来的値  
        }
        if (Request.QueryString["c_uname"] != null && Request.QueryString["c_uname"] != "")
        {
            uname = Request.QueryString["c_uname"].ToString(); //Searchpage面传递过来的値  
        }
        if (Request.QueryString["c_address"] != null && Request.QueryString["c_address"] != "")
        {
            uaddress = Request.QueryString["c_address"].ToString(); //Searchpage面传递过来的値
        }
        if (Request.QueryString["c_tel"] != null && Request.QueryString["c_tel"] != "")
        {
            utel = Request.QueryString["c_tel"].ToString(); //Searchpage面传递过来的値    
        }
    }
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            div_a.InnerHtml = "<script>alert('Please select payment Trademark！');</script>";

        }
    }
    public string ZTFileImg(object zhuti,object sbid)
    {
        string aa = "<a href='user_sbzl.aspx'>Not uploaded</a>";
        var m = DALM.Member_Select_Id(UserId);
        if (m != null)
        {
            if (m.i_UserTypeId == 3)
            {
                if (zhuti != null && zhuti != "")
                {
                    return "<a href='../" + zhuti + "' target='_blank'><img src='../" + zhuti + "'  width='20' title='Certificate of Subject Qualification' border='0'/></a>";
                }
                else
                {
                    return "<a href='user_sbupdate.aspx?t_r_id=" + sbid + "' target='_blank'>Not uploaded</a>";
                }
            }
            else
            {
                ...
                {
                    return "<a href='../" + m.nvc_ZhuTiFile + "'  target='_blank'><img src='../" + m.nvc_ZhuTiFile + "'  width='20' title='Certificate of Subject Qualification' border='0'/></a>";
                }
                else
                {
                    return "<a href='user_sbzl.aspx'>Not uploaded</a>";
                }
            }
        }
        return aa;
    ...
    public string GetSBtypeAndName(string country, string typeid)//获得商标类型
    {
        ...
        if (country == "1")
        {
            if (typeid == "1")
            {
                typename = "Chinese individuals";
            }
            if (typeid == "2")
            {
                typename = "Chinese Enterprise";
            }
            if (typeid == "3")
            {
                typename = "Chinese agency";
            }
        }
        if (country == "2")
        {
            if (typeid == "1")
            {
                typename = "Foreign individuals";
            }
            if (typeid == "2")
            {
                typename = "Foreign company";
            }
            if (typeid == "3")
            {
                typename = "Foreign agents";
            }
        }
        return typename;
    ...
    protected void ImageButton5_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            div_html.InnerHtml = "<script>alert('Please select the Trademark to be deleted.')</script>";
        }
    }
