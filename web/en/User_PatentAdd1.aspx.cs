...
public partial class User_PatentAdd1 : System.Web.UI.Page
{
    ...
    protected void ImgBtn_Add_Click(object sender, ImageClickEventArgs e)//添加修改专利
    {
        ...
        if (model.i_AnnualFeeStatus != 13)//不是All the annuity fee payments already done
        {
        ...
        if (hd_id.Value != "0")
        {
            ...
            {
                div_a.InnerHtml = "<script>alert('The patent number cannot be repeated')</script>";
            }
        }
        else
        {
            if (DALP.Patent_Select_Number(model.nvc_Number, model.i_AuthorizeNationalityId, model.i_MemberId, 0) <= 0)
            {
                ...
                if (DALP.Patent_Add(model) > 0)
                {
                    ...
                    Response.Redirect("User_PatentAddOk.aspx?AddType=Quantity&AddOkCount=1");
                }
            }
            else
            {
    ...
    //        {
    //            ListItem li = new ListItem(q.nvc_EnglishName, q.i_Id.ToString());
