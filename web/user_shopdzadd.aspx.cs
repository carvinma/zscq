...
public partial class user_shopdzadd : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>邮寄地址_账户中心_商城系统_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    protected void Bt_AddAddress_Click(object sender, EventArgs e)
    {
        ...
        {
            div_a.InnerHtml = "<script>alert('请输入收货人!');</script>"; return;
        }
        if (this.ddl_guojia.SelectedValue == "0" )
        {
            div_a.InnerHtml = "<script>alert('请选择国家!');</script>"; return;
        }
        ...
        if (Hi_AddressId.Value == "0")
        {
            switch (DALRA.ReceiveAddress_Add(model))
            {
                ...
                case 1:
                    ...
                    div_a.InnerHtml = "<script>alert('添加成功！'); window.location='user_shopdz.aspx'</script>";
                    break;
                case 2:
        ...
        else
        {
            switch (DALRA.ReceiveAddress_Update(model))
            {
                ...
                case 1:
                    ...
                    div_a.InnerHtml = "<script>alert('修改成功！'); window.location='user_shopdz.aspx'</script>";
                    break;
                case 2:
                    div_a.InnerHtml = "<script>alert('您已经有相同收货人和相同地址信息，不需要重复添加!');</script>";
                    break;
