...
public partial class faq : System.Web.UI.Page
{
    ...
    {
        t_PageInfo model = new dal_PageInfo().PageInfo_Select_Id(50);
    ...
    void Bind_Rpt_List()
    {
        ...
        var iquery = from i in dvdc.vw_OnlineMessage where i.nvc_Language=="jp" select i;
        if (!string.IsNullOrWhiteSpace(Request.QueryString["pagekey"]))
        {
            ...
            {
                lb_msg.Text = "お客様の記入したキャラクターが規則に適合してなく、確認してからもう一度記入してください！";
                return;
            }
    ...
    protected void ibtn_Add_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (DALOM.OnlineMessage_Add(model) > 0)
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.GetType(), "msg", "<script>alert(\"ご質問が既にサービスセンターに発送しました。我々が24時間以内にご返答を致します。直ちに登録し、チェックしてください\");window.location='faq.aspx';</script>", false);
        else
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.GetType(), "msg", "<script>alert(\"メッセージを発送出来ませんでした！\");</script>", false);
