...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    int uId = 0;
    public int uType = 0;
    public int Power_Into = 0, Power_Export = 0, Power_Update = 0, Power_Delete = 0;//四个变量分别代表权限 导入、导出、修改、削除
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        if (Request.Cookies["hqht_patentidstr"] != null && Request.Cookies["hqht_patentidstr"].Value != "")
        {
            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    {
                        div_html.InnerHtml = "<script>alert('[" + tp.nvc_Number + "]該当特許が既に6ヶ月間期限過ぎで、オーダーを提出することができません。もし今年の費用が既に納付済みであれば、納付済み年度を編集してください。')</script>";
                        return;
                    }
        ...
        {
            div_html.InnerHtml = "<script>alert('費用を納付する特許を選択してください')</script>";
        }
    }
    ...
    void Bind_Drp_PatentNationality()
    {
        ...
        ListItem item = new ListItem("特許授権国", "0");
    ...
    private IQueryable<vw_Patent> UserPatent_SelectPage(int pageindex, int pagesize, int uId, ref int count)
    {
        try
        {
            ...
            {
                iquery = from i in iquery where i.nvc_JapaneseName.Contains(Request["nvc_PatentTypeName"]) select i;
            }
    ...
    protected void ImageButton3_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            div_html.InnerHtml = "<script>alert('削除する特許を選択してください')</script>";
        }
    }
}