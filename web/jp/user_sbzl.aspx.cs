...
public partial class user_sbzl : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>資料を修正_アカウントセンター_商標システム_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    public void Bind_Page_Info()//绑定基本情報
    {
        ...
        if (model != null)
        {
            ...
            {
                zhutiwenjian = "<a href='../" + model.nvc_ZhuTiFile + "' target='_blank' title='クリックしてチェックできる'>アップロード済(クリックしてチェックできる)</a><br><span  style=\"color:Red;\">証明書類が変更された場合、再度アップロードしてください。";
            }
        }
    }
    ...
    protected void Bt_Submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (result.Count() > 0)
            {
                div_a.InnerHtml = "<script>alert('電子メールは既に使用され！');</script>";
                return;
            }
            ...
            if (result2.Count() > 0)
            {
                div_a.InnerHtml = "<script>alert('携帯電話の番号は既に使用され！');</script>";
                return;
            }
        }
            if (hi_guoji.Value == "1")
            ...
            if (FileUpload3.HasFile)
            {
                ...
                if (fileNameExt == ".jpg" || fileNameExt == ".gif" || fileNameExt == ".png" || fileNameExt == ".pdf")
                {
                    ...
                    {
                        MessageBox.Show(this.Page, "ファイルサイズは500KB以内に");
                        // div_a.InnerHtml = "<script>alert('文件大小不能超过 512KB');</script>";
                        return;
                    }
                    string toFileName = DateTime.Now.ToString("HHmmssff") + new Random().Next(100);
                    string path = System.AppDomain.CurrentDomain.BaseDirectory + "File_Zscq/File_ShangBiao/" + toFileName + "主体資格証明" + fileNameExt;
                    FileUpload3.SaveAs(path);
                    if (model.nvc_ZhuTiFile != null)
                    {
                        DelPhoto.DeleteFile(model.nvc_ZhuTiFile);
                    }
                    model.nvc_ZhuTiFile = "File_Zscq/File_ShangBiao/" + toFileName + "主体資格証明" + fileNameExt;
                }
                else
                {
                    MessageBox.ShowAndRedirect(this.Page, "ファイルフォーマットミス！", "user_sbzl.aspx");
                    return;
                }
            }
            ...
            // div_a.InnerHtml = "<script>alert('修正しました！');location.href='user_sbzl.aspx';</script>";
            ScriptManager.RegisterStartupScript(UpdatePanel2, this.GetType(), "", "<script>alert('修正しました！');location.href='user_sbzl.aspx';</script>", false);
        }
        catch
        {
            ScriptManager.RegisterStartupScript(UpdatePanel2, this.GetType(), "", "<script>alert('パラメータの誤りがある後に提出し、確認してください！');location.href='user_sbzl.aspx';</script>", false);
