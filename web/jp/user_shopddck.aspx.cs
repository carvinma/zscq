...
public partial class user_shopddck : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>注文書の詳細_私の両替オーダー_ショッピングセンターシステム_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    public void Bind_Page_Value()//绑定所有地址
    {
        if (Request.QueryString["orderid"] != null && Request.QueryString["orderid"].ToString() != "")
        {
            ...
            if (model != null && model.i_MemberId.ToString() == Hi_MemberId.Value)
            {
                ...
                Address += string.IsNullOrWhiteSpace(model.nvc_JpanName) == true ? "" : model.nvc_JpanName + "&nbsp;&nbsp;";
                ...
                OrderStatus = DALIO.Set_IntegralOrderState(model.i_State,"jp");
