...
public partial class user_sbupdate : System.Web.UI.Page
{
    ...
    public string Str_Moneysb = "", Str_Monesbmei = "", bizhong = "",zongji="",zongjimei="";//自らの費用を納める
            ...
            if (mm != null)
            {
                if (mm.i_GuoJiId != null)
                {
                    ...
                    if (mm.i_UserTypeId != null)
                    {
                        ...
                        {
                            zhutiwenjian = "<a href='../" + mm.nvc_ZhuTiFile + "'  title='クリックしてチェックできる'  target='_blank'>主体資格証明書が既にアップロードされました</a>";
                            zhutifileIsOk = 1;
                        }
    ...
    {       
           ...
           // sbguoji = Drp_GuoJi.SelectedValue;
               ...
               // model.i_GuoJiId = int.Parse(Drp_GuoJi.SelectedValue);
                ...
                {
                    string urlname = "";
                    int flag= UpfileLoad(FileUpload4, 4, "File_Zscq/File_ShangBiao/", "商標登録証コピー",ref urlname);
                    if(flag==1)
                    {
                        MessageBox.Show(Page, "ファイルサイズは500KB以内に"); 
                        return;
                    }
                    if(flag==2)
                    {
                        MessageBox.Show(Page, "ファイル形式に限る(.jpg  .png  .gif .pdf)!"); 
                        return;
                    }
                    if(flag==3)
                    {
                        MessageBox.Show(Page, "アップロードできませんでした");
                        return;
                    }
                ...
                if (hi_usertype.Value == "3")
                {
                    ...
                    {
                        string urlname = "";
                        int flag = UpfileLoad(FileUpload3, 3, "File_Zscq/File_ShangBiao/", "商標の主体資格証明書", ref urlname);
                        if (flag == 1)
                        {
                            MessageBox.Show(Page, "ファイルサイズは500KB以内に");
                            return;
                ...
                {
                    MessageBox.Show(this.Page, "商標を追加できませんでした");
                    //div_a.InnerHtml = "<script>alert('商標を追加できませんでした!');<script>";
                }                   
    }
    public int UpfileLoad(FileUpload fileup, int uploadtype, string pathname, string filename,ref string  urlname)
    {
        try
        {
            ...
            {
                MessageBox.Show(this.Page, "ファイル形式に限る(.jpg  .png  .gif .pdf)!");
                return 2;
            }
        }
        catch
