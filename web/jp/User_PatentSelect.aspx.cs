...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    void Bind_PatentType(int typeid)
    {
        ...
        foreach (var q in iquery)
        {
            this.Drp_PatentType.Items.Add(new ListItem(q.nvc_JpanName, q.i_Id.ToString()));
        }
    }
    void Bind_PatentType()
    {
        ...
        ListItem item = new ListItem("選択して下さい", "0");
        Drp_PatentType.Items.Add(item);
        var iquery = DALPT.GetPatentType("jp");
        foreach (var q in iquery)
        {
            this.Drp_PatentType.Items.Add(new ListItem(q.nvc_JapaneseName, q.i_Id.ToString()));
