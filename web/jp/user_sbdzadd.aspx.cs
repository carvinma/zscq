...
public partial class user_sbdzadd : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>書類郵送住所_アカウントセンター_特許システム_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
        ...
        if (Hi_AddressId.Value == "0")
        {
            switch (DALRA.ReceiveAddress_Add(model))
            {
                ...
                case 1:
                    ...
                    div_a.InnerHtml = "<script>alert('追加しました！'); window.location='user_sbdz.aspx'</script>";
                    break;
                case 2:
        ...
        else
        {
            switch (DALRA.ReceiveAddress_Update(model))
            {
                ...
                case 1:
                    ...
                    UserLog.AddUserLog(Hi_MemberId.Value, "商標システム", "添加地址");
                    div_a.InnerHtml = "<script>alert('修正しました！'); window.location='user_sbdz.aspx'</script>";
                    break;
                case 2:
                    div_a.InnerHtml = "<script>alert('同じ受取人と同じ住所が既に存在し、重複に追加しないでください!');</script>";
                    break;
