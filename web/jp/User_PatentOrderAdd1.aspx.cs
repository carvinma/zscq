...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Request.Cookies["hqhtshop"] != null && Request.Cookies["hqhtshop"]["hqht_zl_uid"] != null && Request.Cookies["hqhtshop"]["hqht_zl_uid"].ToString() != "")
        {
            ...
            if (tn != null)
            {
                hi_paybizhong.Value = tn.nvc_JFBizhong;//费用缴纳国家貨幣
            }
            #endregion
            Delete_Patent();
            Bind_PatentList();
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            div_a.InnerHtml = "<script>alert('金額総計が0ではいけません')</script>";
            return;
        }
        #region 生成订单
        ...
        if (rb_kp2.Checked)
        {
            ...
            + "<br />联系电话：" + address.nvc_MobilePhone + "&nbsp;&nbsp;" + address.nvc_TelPhone + "<br />郵便番号编码：" + address.nvc_ZipCode + "<br />メールボックス：" + address.nvc_Email;
            ...
            if (flag != null && flag != "")
            {
                ...
                for (int j = 0; j < flaglist.Length; j++)
                {
                    if (flaglist[j] != "" || flaglist[j] != null)
                    {
                        ...
                        if (t != null && tc != null)
                        {
                            youhui += "+" + t.nvc_JPName + hi_paybizhong.Value + DALPO.GetPayMoneyByHuiLv(Convert.ToDecimal(t.i_Money1), "USD", hi_paybizhong.Value).ToString("0.00");
        ...
        if (Request.Cookies["hqht_patentidstr"] != null && Request.Cookies["hqht_patentidstr"].Value != "")
        {
            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null)
                    {
                        ...
                        InfoModer.nvc_Year = model.nvc_AnnualFeeNotPaymentYear;//納付待ち年度

                        InfoModer.dm_ZhengFuGuiDingFei = model.dm_AnnualFeeNextPaymentMoney;
                        if (shuzu.IndexOf("," + model.i_AuthorizeNationalityId + ",") < 0)//授权国一样的话 只计算一次振込手数料
                        {
                        ...
                        InfoModer.nvc_PatentShouQuanGuo = DALN.Nationality_Select_Id(model.i_AuthorizeNationalityId).nvc_JpanName;
            ...
            strzl.Append("<tr>");
            strzl.Append("<td height='35' align='center' bgcolor='#FFFFFF'>特許番号</td>");
            strzl.Append("<td width='12%' align='center' bgcolor='#FFFFFF'>特許名称</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>特許権利人</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>授権国</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>納付年度</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>納付期限</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>納付金額</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>貨幣</td>");
            strzl.Append("<td align='center' bgcolor='#FFFFFF'>公文書番号</td>");
            strzl.Append("</tr>");
            ...
            zongji.Append("<td width='200' align='right'> 政府規定費用：</td><td width='110'>" + OrderModer.dm_SetMoney.ToString("0.00") + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 延長金：</td><td width='110'>" + OrderModer.dm_ZhiNaJin + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 代理費：</td><td width='110'>" + OrderModer.dm_AgentServiceFee.ToString("0.00") + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 項目超過費用：</td><td width='110'>" + OrderModer.dm_ChaoXiangFee.ToString("0.00") + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'></td><td width='110'>小計：</td><td width='30' style='color: red;'>" + OrderModer.nvc_CurrencyType + "</td><td style='width: 100px;color: red;'>" + (Convert.ToDecimal(OrderModer.dm_SetMoney + OrderModer.dm_AgentServiceFee + OrderModer.dm_ChaoXiangFee + OrderModer.dm_ZhiNaJin)).ToString("0.00") + "</td>"); 
            zongji.Append("</tr>");
            ...
            { 
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> クーポン券を使用する：</td><td width='110'>" + OrderModer.dm_YouHui + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
			}
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'> 振込手数料：</td><td width='110'>" + OrderModer.dm_RemittanceFee + "</td><td width='30'></td><td style='width: 100px;'></td>");
            zongji.Append("</tr>");
            ...
            zongji.Append("<table width='440' cellspacing='0' cellpadding='0' border='0'><tbody>");
            zongji.Append("<tr align='left'>");
            zongji.Append("<td width='200' align='right'></td><td width='110'>総計：</td><td width='30' style='color: red;'>" + OrderModer.nvc_CurrencyType + "</td><td style='width: 100px;color: red;'>" + OrderModer.dm_TotalMoney.ToString("0.00") + "</td>");
            zongji.Append("</tr>");
            zongji.Append("</tbody></table>");
            ...
            str = str.Replace("$order_type", "特許注文");
            str = str.Replace("$order_stat", DALPO.GetStatus(OrderModer.i_Status,"jp"));
            str = str.Replace("$user_num", tm.nvc_UserNum);
            str = str.Replace("$true_name", tm.nvc_RealName);
            str = str.Replace("$user_counrtyInfo", DALN.Nationality_Select_Id(tm.i_GuoJiId).nvc_JpanName);
            ...
            if (OrderModer.nvc_BankId != null && OrderModer.nvc_BankId.ToString()!="")
            {
                ...
                yinhang.Append("<tr>");
               yinhang.Append("<td width='24%' height='28' align='center' bgcolor='#FFFFFF'><strong>銀行の詳細</strong></td>");
                yinhang.Append("<td width='20%' height='28' align='center' bgcolor='#FFFFFF'><strong>スウィフトコード</strong></td>");
                yinhang.Append("<td width='28%' height='28' align='center' bgcolor='#FFFFFF'><strong>アカウント名</strong></td>");
                yinhang.Append("<td width='28%' height='28' align='center' bgcolor='#FFFFFF'><strong>口座番号</strong></td>");
                yinhang.Append("</tr>");
            ...
            if (remail == 1)//如果会员接收邮件并且后台设置发送邮件
            {
                ...
                string send_fromtitle = "環球汇通-特許システム-注文書を提出して";//标题
            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null)
                    {
                        ...
                        sb_patent.Append("<td align='center' bgcolor='#FFFFFF'>" + DALN.Nationality_Select_Id(model.i_AuthorizeNationalityId).nvc_JpanName + "</td>");
                        ...
                        sb_patent.Append("<td align='center' bgcolor='#FFFFFF'><a href='User_PatentInfo.aspx?pId=" + pid + "&href=" + href + "' class='ac5t'>チェック</a> <a href=" + "'?pagetype=del&cs=" + pid + "," + number + "' class='ac5t'>削除</a></td>");
                        sb_patent.Append("</tr>");
                        ...
                        sb_patent_1.Append("<td align='center' bgcolor='#FFFFFF'>" + DALN.Nationality_Select_Id(model.i_AuthorizeNationalityId).nvc_JpanName + "</td>");
                        ...
                        if (model.i_AuthorizeNationalityId != 1)
                        {
                            ...
                            {
                                //税率=(（政府規定費用+代理費+振込手数料+滞纳金+項目超過費用）换算成人民币-200 )*33/1000
                        ...
                        //有个问题：同一授权国下的几个专利振込手数料不一样的时候按哪个来
                    }
                    #endregion
                }
            }
