          ...
          <tr>
            ...
            <td>
              ...
              <%#Eval("nvc_StreetAddress")%>
               &nbsp;&nbsp;（郵便番号:<%# Eval("nvc_ZipCode")%>）
              &nbsp;&nbsp;（<%# Eval("nvc_Consignee")%>&nbsp;&nbsp;收）&nbsp;&nbsp;<%# Eval("nvc_TelPhone")%>&nbsp;&nbsp;
              <asp:LinkButton ID="lbtn_SetMr" runat="server" CommandName="SETMR" CommandArgument='<%# Eval("i_Id")%>'>[削除]</asp:LinkButton>
              &nbsp;&nbsp;
              <asp:LinkButton ID="lbtn_Delete" runat="server" CommandName="DELETE" CommandArgument='<%# Eval("i_Id")%>' OnClientClick="javascript:if(!window.confirm('削除の確認い？')){return false;}">[削除]</asp:LinkButton>
            </td>
          </tr>
        </ItemTemplate>
      </asp:Repeater>
