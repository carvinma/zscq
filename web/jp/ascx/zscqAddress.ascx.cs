...
public partial class ascx_zscqAddress : System.Web.UI.UserControl
{
    ...
    void Mr_Address(int addrId)//默认
    {
        ...
        {
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.Page.GetType(), "msg", "<script>alert('事故の発生！')</script>", false);
        }
    }
    void Delete_Address(int addrId)//削除地址
    {
        if (DALRA.ReceiveAddress_Del2(addrId) == 1)
        {
            UserLog.AddUserLog(MemberId, "地址管理", "删除地址");
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.Page.GetType(), "msg", "<script>alert('削除しました！')</script>", false);
            BindValue();
        }
        {
            ScriptManager.RegisterStartupScript(UpdatePanel1, this.Page.GetType(), "msg", "<script>alert('事故が起こり、このアドレスを削除することができない！')</script>", false);
        }
    }
    ...
    protected void ibtn_Add_Click(object sender, ImageClickEventArgs e)//添加地址
    {
        try
        {
            ...
            {
                case 0:
                    ScriptManager.RegisterStartupScript(UpdatePanel1, this.Page.GetType(), "msg", "<script>alert('事故の発生，当分添加アドレス！')</script>", false);
                    break;
                case 1:
                    if (CB_Mr.Checked)
                    {
                    ...
                    BindValue();
                    ScriptManager.RegisterStartupScript(UpdatePanel1, this.Page.GetType(), "msg", "<script>alert('追加しました！')</script>", false);
                    break;
                case 2:
                    ScriptManager.RegisterStartupScript(UpdatePanel1, this.Page.GetType(), "msg", "<script>alert('同じ受取人と同じ住所が既に存在し、重複に追加しないでください!')</script>", false);
                    break;
            }
        }
        catch { }
