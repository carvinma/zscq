...
public partial class user_sbmm : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>パスワードを修正_アカウントセンター_商標システム_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    ...
    protected void Bt_Submit_Click(object sender, EventArgs e)//パスワードを修正
    {
        ...
        {
            div_a.InnerHtml = "<script>alert('旧いパスワードが正しくありません！')</script>";
            return;
        }
        ...
        UserLog.AddUserLog(Hi_MemberId.Value, "商標システム", "パスワードを修正");
        
        div_a.InnerHtml = "<script>alert('修正しました！次回の登録で発効します！');</script>";
    }
}