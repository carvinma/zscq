...
public partial class user_sblb : System.Web.UI.Page
{
    ...
    public void Bind_Page_value()
    {
        ...
        {
            isjiaofei = int.Parse(Request.QueryString["isjiaofei"].ToString()); //统计页面传递过来的値 費用を納付するかどうか     
        }
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            div_a.InnerHtml = "<script>alert('納付商標を選択してください');</script>";

        }
    }
    public string ZTFileImg(object zhuti,object sbid)
    {
        string aa = "<a href='user_sbzl.aspx'>まだアップロードしていない</a>";
        var m = DALM.Member_Select_Id(UserId);
        if (m != null)
        {
            if (m.i_UserTypeId == 3)
            {
                if (zhuti != null && zhuti != "")
                {
                    return "<a href='../" + zhuti + "' target='_blank'><img src='../" + zhuti + "'  width='20' title='主体資格証明書類' border='0'/></a>";
                }
                else
                {
                    return "<a href='user_sbupdate.aspx?t_r_id=" + sbid + "' target='_blank'>未アップロード</a>";
                }
            }
            else
            {
                ...
                {
                    return "<a href='../" + m.nvc_ZhuTiFile + "'  target='_blank'><img src='../" + m.nvc_ZhuTiFile + "'  width='20' title='主体資格証明書類' border='0'/></a>";
                }
                else
                {
                    return "<a href='user_sbzl.aspx'>未アップロード</a>";
                }
            }
        }
        return aa;
    ...
    protected void ImageButton5_Click(object sender, ImageClickEventArgs e)
    {
        if (Request.Cookies["hqht_Trademarktidstr"] != null && Request.Cookies["hqht_Trademarktidstr"].Value != "")
        {
            ...
            if (model != null)
            {
                if (model.nvc_Power != null && model.nvc_Power != "")
                {
                    booldelete = model.nvc_Power.Split(',')[3] == "1" ? true : false;//削除  
            ...
            Bind_Rpt_Trademark();
            div_html.InnerHtml = "<script>alert('削除しました')</script>";
        }
        else
        {
            div_html.InnerHtml = "<script>alert('削除する商標を選択してください')</script>";
        }
    }
