...
function commentallpage(vpage) {
  obj('Y_Foot_Page1').innerHTML = "<img alt=\"少々お待ちください\" src=\"images/loading.gif\" style=\"margin-left:350px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function zxpage(vpage) {
  obj('Y_Foot_Page2').innerHTML = "<img alt=\"少々お待ちください\" src=\"images/loading.gif\" style=\"margin-left:340px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function buynotepage(vpage) {
  obj('divbuynote').innerHTML = "<img alt=\"少々お待ちください\" src=\"images/loading.gif\" style=\"margin-left:340px;\" />";
  var ProId = obj('Hi_PId');
  var rand = Math.floor(Math.random() * 1000000);
...
function commentsubmit() {
  if (obj("t_content").value == "文明はインターネットを利用して、理誌発言。" || obj("t_content").value.toString() == "") {
    obj('t_content').focus();
...
//登录
function GetLogin() {
  if (obj("text_username").value == "ユーザー名" || obj("text_username").value.toString() == "") {
    obj('text_username').focus();
    changecolor('div_username', 0, "ユーザー名を記入してください");
    return false;
  }
  if (obj("TB_pwd").value == "") {
    ...
    obj('TB_pwd').focus();
    changecolor('div_pwd', 0, "パスワードを記入してください！");
    return false;
  }
  if (obj("text_v")) {
    if (obj("text_v").value == "验证码" || obj("text_v").value.toString() == "") {
      obj('text_v').focus();
      changecolor('div_checkcode', 0, "検証コードを記入してください！");
      return false;
    }
  }
...
function checkonsubmit() {
  if (!obj("cb_Agreement").checked) {
    alert("申し訳ありませんが、本サイトの会員登録すれば、必ずご同意会員登録協議！");
    return false;
  }
  //    if (obj("text_regmail").value.toString() == "" || obj('text_regmail').value == "邮箱/手机") {
  //        obj("text_regmail").focus();
  //        changecolor('div_regmail', 0, "ユーザー名を記入してください", 'li_tixing2');
  //        return false;
  //        }
  ...
  if (reg.test(obj("text_regmail").value.toString()) == false && reg3.test(obj('text_regmail').value) == false) {
    ...
    changecolor('div_regmail', 0, "ユーザー名フォーマットの誤りがある！", 'li_tixing2');
    return false;
  }
  ...
  if (obj('Hi_Repeat').value == "1") {
    obj("text_regmail").select();
    changecolor('div_regmail', 0, obj("text_regmail").value + "すでに存在する！", 'li_tixing2');
    return false;
  }
  ...
  if (obj("pwd_regpwd").value.toString() == "") {
    ...
    changecolor('div_regpwd', 0, "パスワードを記入してください！");
    return false;
  }
  if (obj("pwd_regpwd").value.length < 6 || obj("pwd_regpwd").value.length > 21) {
    ...
    obj('text_regpwd').style.display = 'none';
    changecolor('div_regpwd', 0, "パスワードの長さが6〜21桁の間に！");
    return false;
  }
  if (obj("pwd_regpwdok").value.toString() == "") {
    ...
    changecolor('div_regpwdok', 0, "パスワードを再入力してください！");
    return false;
  }
  if (obj("pwd_regpwd").value.toString() != obj("pwd_regpwdok").value.toString()) {
    ...
    obj('text_regpwdok').style.display = 'none';
    changecolor('div_regpwdok', 0, "二次入力したパスワードと違って！");
    return false;
  }
  if (obj('text_regcheckcode').value == "" || obj('text_regcheckcode').value == "验证码") {
    obj("text_regcheckcode").focus();
    changecolor('div_regcheckcode', 0, "検証コードを記入してください！");
    return false;
  }
  obj('IBt_Register').innerHTML = "登録している…";
}
