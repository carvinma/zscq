...
public partial class user_sbck : System.Web.UI.Page
{
    ...
    public string zhutifileok = "まだアップロードしていない", sbfileok = "まだアップロードしていない", shenqingfile = "まだアップロードしていない", weituofile = "まだアップロードしていない", Uname = "";
    public string Str_Moneysb = "", Str_Monesbmei = "", Str_Zhinajin = "", bizhong = "", zongji = "", zongjimei = "";//自らの費用を納める
    ...
    public string daoqitime = "", shenhe = "まだチェックしていない";
    ...
    private void Bind_Page_Info(int trademarkId)// 绑定商标详细数据
    {
        ...
        if (model != null)
        {
            ...
            if (mm != null)
            {
                ...
                {
                    zhutifileok = "<a href='../" + mm.nvc_ZhuTiFile + "' title='クリックしてチェックできる' target='_blank'>主体資格証明書が既にアップロードされました</a>";
                }
                ...
                if (mm.i_UserTypeId == 3)//代理
                {
                    ...
                    {
                        zhutifileok = "<a href='../" + model.nvc_ZhuTiFile + "' title='クリックしてチェックできる' target='_blank'>主体資格証明書が既にアップロードされました</a>";
                    }
                    else
                    {
                        zhutifileok = "<a href='user_sbupdate.aspx?t_r_id=" + trademarkId + "' target='_blank'>まだアップロードしていない</a>";
                    }
            ...
            SBPssTime = model.nvc_SbRegTime;
                ...
                JiaoFeiType = model.i_JiaoFeiType.ToString();// ? "自らの費用を納める" : "代理缴费";
            //if (model.nvc_ZhuTiFile != null)
            //{
            //    zhutifileok = "<a href='" + model.nvc_ZhuTiFile + "' title='クリックしてチェックできる' target='_blank'>主体資格証明書が既にアップロードされました</a>";
            //}
            ...
            {
                shangchuan = 1;//是否上传
                sbfileok = "<a href='../" + model.nvc_SBFile + "' title='クリックしてチェックできる' target='_blank'>商標登録証明書のコピーはアップロード</a>";
            }
            if (model.nvc_ShenQingShu != null)
            {
                //shangchuan = 2;//是否上传
                shenqingfile = "<a href='../" + model.nvc_ShenQingShu + "' title='クリックしてチェックできる' target='_blank'>申請書をアップロード</a>";
            ...
            if (model.i_JiaoFeiType == 2)
            {
                ...
                {
                    weituofile = "<a href='../" + model.nvc_WeituoFile + "' title='クリックしてチェックできる' target='_blank'>依頼書をアップロード</a>";
                    weituoshu = 1;
                    //Button1.Visible = false;
                }
                else
    ...
    protected void Button1_Click1(object sender, EventArgs e)
    {
        ...
        //    {
        //        MessageBox.Show(this.Page, "アップロードしてください依頼書と申請書のコピー!");
        ...
        //    else
        //    {
        //        MessageBox.Show(this.Page, "商標申請書をアップロードしてください!");
        //        // div_a.InnerHtml = "<script>alert('请选择你要上传的文件');</script>";
        //        return;
        //    }
        //}
    ...
    public void UpfileLoad(FileUpload fileup, string pathname, string filename, ref string urlname)
    {
        ...
        if (fileNameExt == ".jpg" || fileNameExt == ".gif" ||fileNameExt == ".png"  || fileNameExt == ".pdf")
        {
            ...
            {
                div_a.InnerHtml = "<script>alert('ファイルサイズは500KB以内に');</script>";
                return;
            }
            ...
            MessageBox.Show(this.Page, "アップロードして！");
        }
        else
        {
            MessageBox.Show(this.Page, "ファイル形式に限る( .jpg .gif .pdf)!");
