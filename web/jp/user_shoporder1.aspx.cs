...
public partial class user_shoporder1 : System.Web.UI.Page
{
    ...
    public void Bind_Page_Title()
    {
        ...
        TitleOrKeyword = "<title>ポイント両替情報確認_" + ShopName + "</title><meta name=\"Keywords\" content=\"" + ShopKeywords + "\"/><meta name=\"Description\" content=\"" + ShopDescription + "\"/>";
    }
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            DALN.Nationality_Bind_DDL(ddl_guojia,"jp");
            Bind_Page_Member();
            Bind_Page_Value();
                ...
                foreach (var q in ShopCart)
                {
                    if (q != "")
                    {
                        ...
                        if (model != null)
                        {
                            ...
                            td_ShopCart.Append("<td width='69' align='center' bgcolor='#FFFFFF'><a target=\"_blank\" href='user_shoporder1.aspx?action=del&cid=" + model.i_Id + "&pid=" + model.i_Type + "' class='ac5t'>削除</a></td>");
                            td_ShopCart.Append("</tr>");
                        }
                    }
                }
        ...
        try
        {
            ...
            if (m_member != null & m_mobile != null)
            {
                if (m_mobile.i_Integral >= CCart.Set_ShopCartIntegral())
                {
                    ...
                    {
                        div_a.InnerHtml = "<script>alert('请先去选择商品！');window.location='jifen.aspx';</script>";
                        return;
                    }
                    int sumgrade = 0;
                    foreach (var q in ShopCart)
                        ...
                        if (q != "")
                        {
                            ...
                            if (model_ip != null)
                            {
                                if (model_ip.i_Type == 1)
                                {
                                    ...
                                    {
                                        div_a.InnerHtml = "<script>alert('请先选择地址！');window.location='user_shoporder1.aspx';</script>";
                                        return;
                                    }
                                    #endregion
                                }
                                ...
                                if (model_ip.i_Type == 3)
                                {
                                    ...
                                    {
                                        div_a.InnerHtml = "<script>alert('会员等级商品只能购买一件！');window.location='user_shoporder1.aspx';</script>";
                                        return;
                                    }
                                    if (m_member.i_GradeId >= model_ip.i_Grade)
                                    {
                                        div_a.InnerHtml = "<script>alert('无需购买此会员等级商品！');window.location='user_shoporder1.aspx';</script>";
                                        return;
                                    }
                                    if (model_ip.i_Grade - m_member.i_GradeId > 1)
                                    {
                                        div_a.InnerHtml = "<script>alert('不能购买此会员等级商品！');window.location='user_shoporder1.aspx';</script>";
                                    }
                                    #endregion
                                }
                ...
                {
                    div_a.InnerHtml = "<script>alert('积分不足');window.location='user_shoporder1.aspx';</script>";
                    return;
                }
            }
            else
            {
                div_a.InnerHtml = "<script>alert('登录已过期！');window.location='index.aspx?pageurl2='" + HttpUtility.UrlEncode(Request.Url.ToString()) + "</script>";
            }
        }
        catch
        {
