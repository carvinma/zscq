...
public partial class user_sbupdate : System.Web.UI.Page
{
    ...
    public string JiaoFeiType = "0", Sb_num = "", shenhe = "まだチェックしていない", jiaofei = "まだ費用を納付してない";
    ...
    public string Str_Moneysb = "0", Str_Monesbmei = "", Str_Zhinajin = "", bizhong = "", zongji = "", zongjimei = "";//自らの費用を納める
    public string Str_Moneysbweituo = "", Str_Monesbmeiweituo = "", sbdailiweituo = "", sbmeidailiweituo = "", zongjiweituo = "", zongjimeiweituo = "";//委託納付
        ...
        else
        {
            ...
            if (Request.QueryString["t_r_id"] != null && Request.QueryString["t_r_id"].ToString() != "")
            {
                ...
                if (model != null)
                {
                    ...
                    if (mm != null)
                    {
                        ...
                        if (mm.i_UserTypeId != null)
                        {
                            if (mm.i_UserTypeId == 1)
                            {
                                ...
                                {
                                    zhutiwenjian = "<a href='../" + mm.nvc_ZhuTiFile + "' title='クリックしてチェックできる'  target='_blank'>主体資格証明書が既にアップロードされました</a><br><span  style=\"color:Red;\">証明書類が変更された場合、再度アップロードしてください。</span>";
                                    zhitiOk = 1;
                                }
                                else
                                {
                            ...
                            if (mm.i_UserTypeId == 3)
                            {
                                ...
                                {
                                    zhutiwenjian = "<a href='../" + model.nvc_ZhuTiFile + "' title='クリックしてチェックできる'  target='_blank'>主体資格証明書が既にアップロードされました</a><br><span  style=\"color:Red;\">証明書類が変更された場合、再度アップロードしてください。</span>";
                                    zhitiOk = 1;
                                }
                                #region 第三方情報
                                t_anjuanhao.Value = model.nvc_CFaxnumber;//公文書番号
                    ...
                    {
                        Sbfile = "<a href='../" + model.nvc_SBFile + "' title='クリックしてチェックできる'  target='_blank'>商標登録証明書のコピーはアップロード</a><br><span  style=\"color:Red;\">証明書類が変更された場合、再度アップロードしてください。</span>";
                        sbfileok = 1;
                    }
                    else
                    {
                    ...
                    {
                        //shangchuan = 2;//是否上传
                        shenqingfile = "<a href='../" + model.nvc_ShenQingShu + "' title='クリックしてチェックできる' target='_blank'>申請書をアップロード</a>&nbsp;&nbsp;<span  style=\"color:Red;\">証明書類が変更された場合、再度アップロードしてください。</span>";
                    ...
                    if (model.i_JiaoFeiType == 2 || mm.i_UserTypeId == 3)
                    {
                        ...
                        {
                            weituofile = "<a href='../" + model.nvc_WeituoFile + "' title='クリックしてチェックできる' target='_blank'>依頼書をアップロード</a>&nbsp;&nbsp;<span  style=\"color:Red;\">証明書類が変更された場合、再度アップロードしてください。</span>";
                            weituoshu = 1;
                            //Button1.Visible = false;
                        }
                        else
                    ...
                    shengtime = model.i_ShengDays.ToString() + "日";
                    if (model.i_ShengDays < 0)
                    {
                        shengtime = "期限が切れる";
            ...
            if (Request.QueryString["t_r_id"] != null && Request.QueryString["t_r_id"].ToString() != "")
            {
                ...
                if (model.nvc_SBFile == "" || model.nvc_SBFile == null)
                {
                    ...
                    //{
                        ...
                        if (i == 1)
                        {
                            div_a.InnerHtml = "<script>alert('ファイルサイズは500KB以内に');location.href='" + url + "';</script>";
                            return;
                        }
                        if (i == 2)
                        {
                            div_a.InnerHtml = "<script>alert('ファイル形式に限る(.jpg  .png  .gif .pdf)!');location.href='" + url + "';</script>";
                            return;
                        }
                    ...
                    //else
                    //{
                    //    div_a.InnerHtml = "<script>alert('商標登録証明書のコピーをアップロードしてください');location.href='" + url + "';</script>";
                    //   // MessageBox.ShowAndRedirect(this.Page, "请上传商标注册证书复印件",url);
                ...
                {
                    #region 第三方情報
                    model.nvc_CFaxnumber = t_anjuanhao.Value;//公文書番号
                    ...
                    //    else
                    //    {
                    //        MessageBox.Show(this.Page, "请上传主体资格证书");
                    //        //  div_a.InnerHtml = "<script>alert('请选择你要上传的文件');</script>";
                    //        return;
                    //    }
                    //}
                ...
                if (DALT.Trademark_Update(model) == 1)
                {
                    ...
                    UserLog.AddUserLog(TrademarkId, "商标系统", "更新商标内容");
                    Response.Redirect("user_sblb.aspx");
                }
                else
                {
                    div_a.InnerHtml = "<script>alert('情報の更新に失敗しました!');<script>";
                }
            }
        }
    }
