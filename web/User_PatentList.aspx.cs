...
public partial class User_PatentCount : System.Web.UI.Page
{
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        if (Request.Cookies["hqht_patentidstr"] != null && Request.Cookies["hqht_patentidstr"].Value != "")
        {
            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    {
                        div_html.InnerHtml = "<script>alert('[" + tp.nvc_Number + "]该专利已经过期6个月，无法提交订单，如本年度已缴费，请重新编辑已缴年度，如需帮助，请联系我们！')</script>";
                        return;
                    }
        ...
        {
            div_html.InnerHtml = "<script>alert('请选择要缴费的专利')</script>";
        }
    }
    ...
    void Bind_Drp_PatentNationality()
    {
        ...
        ListItem item = new ListItem("专利授权国", "0");
    ...
    protected void ImageButton3_Click(object sender, ImageClickEventArgs e)
    {
        if (Request.Cookies["hqht_patentidstr"] != null && Request.Cookies["hqht_patentidstr"].Value != "")
        {
            ...
            string patentid = Request.Cookies["hqht_patentidstr"].Value;
            string[] arr_pid = patentid.Split(new[] { "|" }, StringSplitOptions.RemoveEmptyEntries);
            for (var i = 0; i < arr_pid.Length; i++)
            {
                int pid = Convert.ToInt32(arr_pid[i].Split(',')[0]);
                DALP.Patent_Del(pid);
        ...
        {
            div_html.InnerHtml = "<script>alert('请选择要删除的专利')</script>";
        }
    }
}