            ...
            for (var i = 0; i < arr_pid.Length; i++)
            {
                if (arr_pid[i] != "")
                {
                    ...
                    if (model != null)
                    {
                        ...
                        sb_trademrk.Append("<td width='102' height='32' align='center' bgcolor='#FFFFFF' >" + model.nvc_SBRegNum + "</td>");
                        sb_trademrk.Append("<td width='72' align='center' bgcolor='#FFFFFF'>" +model.nvc_SBType + "</td>");
                        sb_trademrk.Append("<td width='85' align='center' bgcolor='#FFFFFF'>" + (model.i_JiaoFeiType.ToString() == "1" ? "自行缴费" : "委托缴费") + "</td>");
                        sb_trademrk.Append("<td width='78' align='center' bgcolor='#FFFFFF'>" + model.nvc_SBRegName + "<br/>" + (GetGuojiName(model.i_GuoJiId).ToString() != "中国" ? model.nvc_SBRegEnName : "") + "</td>");
                        ...
                        sb_trademrk.Append("<td width='107' align='center' bgcolor='#FFFFFF'>" + model.nvc_SbRegTime + "</td>");
                        sb_trademrk.Append("<td width='67' align='center' bgcolor='#FFFFFF' style='line-height:20px;'>");
                        t_Member tm = DALM.Member_Select_Id(uId);
                        if (tm != null)
                        {
                            if (tm.i_UserTypeId == 3)
                            {
                               sb_trademrk.Append(" <a href=\"user_World_sbweituo.aspx?id=" + tid + "&type=2&zixing=ok\" target=\"_blank\" class=\"ac5t\">委托书下载</a><br>");

                            }
                        }
                        sb_trademrk.Append("<a href=' user_World_sb.aspx?id=" + tid + "&zixing=ok' class='ac5t'>申请书下载</a></td>");
