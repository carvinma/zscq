...
namespace zscq.BLL
{
    ...
    public class bll_Email
    {
        ...
        public int Email_Add(string emailaddress, string title, string _Content, int MId, ref bool State, string lang,string emailAttachments)
        {
            try
            {
                t_SystemKey sys = DALSK.SystemKey_Select_Key("smtp_email", lang);
                ...
                string reWrite = Email.Mail(smtp_email, smtp_name, emailaddress, title, _Content, smtp_username, smtp_password, smtp, emailAttachments);
                t_EmailNote model = new t_EmailNote();
                model.nvc_Title = title;
                model.nvc_EmailContent = _Content;
                model.nvc_EmailState = reWrite;
